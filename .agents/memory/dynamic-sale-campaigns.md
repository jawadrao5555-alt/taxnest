...
- Any DI cycle-toggle price surface (Monthly/Quarterly/Semi-Annual/Annual) must keep the toggle AND the price grid in ONE Alpine `x-data` scope with `calcMonthly`/`calcPrice(base)` reading `sale_price` — mirror `billing/plans.blade.php`. TRAP: if the toggle and grid are separate scopes (or the grid prices are static Blade `number_format`), toggling silently never recalculates — monthly looks right, other cycles don't. Discounts must match `Subscription::getDiscountForCycle` {q:1,sa:3,a:6}.
