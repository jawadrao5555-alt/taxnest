...
- Search divergence (24 Jul 2026): PRA universal search is now STRICT PREFIX on names + codeSearch-gated barcode/SKU substring (see `pos-grid-toggle-search.md`); FBR port still name-includes only and never had barcode/SKU matching. On the next FBR sync, apply strict prefix + the codeSearch gate there too.
- Layout divergence (24 Jul 2026): PRA action bar is now TWO rows (`.tn-action-row1` customer WIDE + order widgets + utility buttons; `.tn-action-row2` category + full-width search + bill-action buttons; cart 320/380/420px; customer partial flex-1 min220/max520). FBR port keeps the old single bar — mirror the split on the next FBR sync (its cart width + mobile CSS child selectors too).

- **Day-close auto-finalize (Aug 2026):** FBR POS reuses `companies.pos_dayclose_provisional_action` ('finalize' = auto-promote; anything else = carry). Sweep lives in FbrPosController::finalizeFbrProvisionalsAtDayClose, runs inside performDayClose BEFORE figures are queried; uses the same atomic local→fbr claim as apiPromoteProvisional (reporting-OFF → fbr/NULL invariant), month-gates previous-month provisionals, agent-mode leaves 'pending' for the desktop agent, cloud failures stay Fail-Queue retryable. Policy UI = a card on /fbr-pos/settings (POST branch `dayclose_pending_update`).
