...
- Gold `#E7BF3B` ONLY as tiny highlights (one word in a headline, a hairline on a dark banner, one badge, one table-header accent). Never large areas. EXCEPTION: on DARK teal sections, the single primary CTA button may be solid gold with dark `#062A33` text — that is the deliberate "punch" accent (hero + final CTA).
- Dark premium treatment (owner's stated taste = dark glassmorphism like the login pages): hero + final CTA + footer are dark teal (`#052730`→`#07333E`→`#0A4D5C` gradients / flat `#052730` footer), white headlines, gray-300/400 body, white/10 glass surfaces, `#2EA0B3` icons/checks, gold hairlines. Middle sections stay light for contrast.
- Owner explicitly LOVES the frosted glass surface recipe (`bg-white/10 + border-white/15-20 + backdrop-blur` on dark teal) — he called the glass button scheme "zyada pyari". Nav is dark glass (`bg-[#052730]/85 + blur`), trust strip under hero is a dark `#063B47` band with glass pills. Prefer this glass recipe for any new element on dark surfaces.
