...
- **Plan gate (Aug 2026):** Custom Access is Unlimited-only (`pricing_plans.custom_access_enabled`). `customSet()` returns null when the plan lacks it → stored sets inert, member behaves as plain role. Inside customSet NEVER lazy-load `$user->company` (live has strict lazy-loading — team page 500'd); use relationLoaded()/Company::find. UI lock-card on /pos/team modal + planAllows check in setCashierAccess.
