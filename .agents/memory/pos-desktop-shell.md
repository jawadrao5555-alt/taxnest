...
## Keep-alive POS window (v1.5.2)
- POS window "close" = **hide, not destroy** (owner rule 26 Jul 2026: "bar bar load na ho, khara rahe") — reopen is instant. Real close ONLY via `forceClose` (closePosWindow) or `opts.isQuitting()` (main.js passes `() => isQuitting`); NEVER remove the isQuitting escape or tray Quit + self-update taskkill hand-off hang.
- `backgroundThrottling: false` on the POS webPreferences — hidden window must keep the 20s incoming-orders poll + offline queue at full speed.
- On every `show`/`restore` (60s throttle) the shell calls `window.tnDesktopResumeCheck` (exposed by universal.blade.php init → bootFpCheck) — deploy while hidden = ONE self-reload on next show; bootFpCheck's busy-guard/one-shot sig guard handle mid-sale + loops. Hook is undefined on login/offline pages (no-op by design).
- One-time-per-run hide Notification (Roman Urdu) tells staff NestPOS is still ready in the background.
...
## Agent auto-config (zero manual setup, v1.5.0 beta2; company-switch beta4)
...
- **beta4 (owner rule, 24 Jul 2026): agent FOLLOWS the POS login.** Seeing /pos/login re-arms the check (`autoCfgDone=false`); on the next authed load main.js re-fetches and compares — same company + same key = no-op (zero writes), different company = full config swap + agent restart, same company with regenerated key = key self-heal. The old "never overwrite an existing complete config" rule is RETIRED — do not re-add it. A switch fires an OS Notification (fiscal_device shops: agent = ONLY PRA path, staff must notice ownership moved); re-arm is race-safe via a generation counter (login-page check runs BEFORE the inFlight gate).
