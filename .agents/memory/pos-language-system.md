...
- Sale screens fully converted; panel pages (POS+FBR dashboards/reports/settings/restaurant, both layouts, controller flash strings) converted 31 Jul 2026 — catalogs now ~3000 keys; receipts still pending. Auth/login pages + QR menu NOT converted.
- Dynamic status labels use `Lang::has('pos.prefix_'.$code) ? __() : ucfirst($code)` (delivery_status_*, table_status_*) — new status codes need a catalog key or fall back to English ucfirst. Receipts: silent-print renders server-side — locale must be set at render time from bill's company/user, not web session.
