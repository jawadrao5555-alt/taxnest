...
- Dashboard card + saveDayOpening key on `PosBusinessDay::current()` (per-company cutoff), NOT calendar today — at 2 AM a late-night shop sees/saves YESTERDAY's opening until that day closes (Jul 2026).
- After-midnight: VERIFIED FIXED (31 Jul 2026, dev test w/ cutoff 04:00) — manual close at 1 AM targets the OPEN trading day (yesterday) via `PosBusinessDay::current` default in both the day-close page GET and closeDayReport POST (hidden date field carries it), so the new day's opening is never locked. FBR POS panel stays calendar-date based end-to-end (no business_date) — internally consistent, separate ecosystem.
