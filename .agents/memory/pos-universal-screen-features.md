...
- **Cart rows v3 (owner + ZFC, 26 Jul 2026 — supersedes v2)**: cart row = qty controls + line total + trash ONLY. Per-item TAX button, per-item Disc button+panel, AND per-item note input ALL REMOVED (v2's icon-button row lasted hours — owner wanted them gone too). Read-only chips row under the item shows carried-in state (green NO TAX / orange -X% / amber note) — the NO TAX chip is the ONLY exempt indicator (T/Alt+T still toggles). D shortcut now opens the BILL discount panel (`$refs.billDiscountInput`). Bill note = single note surface, INSIDE THE PAY MODAL (bound to `kitchenNotes`, rides every payload; `data-pay-note` guard makes modal 1/2/arrow/Enter shortcuts type-safe while focused, Enter/Escape blur). Data fields (`special_notes`/`is_tax_exempt`/`item_discount_*`) stay in cart model + ALL payloads — recalled/edited bills never lose values. Do NOT re-add per-item editors or the cart-footer textarea without owner asking.
...
## Table Board (26 Jul 2026)
- Tables panel below cart + action-menu modal + Final confirm modal — full rules in `pos-restaurant-module.md` (Table Board section). New modal flags `boardMenuTable`/`boardConfirm` are in every plain-letter/F10/F11/Alt gate list — any future modal flag must join them.
...
- Table Board (Jul 2026 improvements): urgency highlighting via boardTileUrgent() (waiter >=10min unclaimed / occupied >=30min / reserved >=15min -> solid stronger tint + warning + pulse on time chip), "Rs X chalu" open-orders sum in strip header (table tiles + tableless chips, no double count — chips filter table_id null), Counter Orders chips for tableless waiter orders via claimAndLoadIncoming, Alt+B board toggle. KNOWN GAP (architect, non-blocking): counter chips get NO urgency highlighting, and waiter urgency uses occupied_since (order-placement time) as unclaimed proxy — future pass.
