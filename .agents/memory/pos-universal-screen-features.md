...
- **Notes+discount one row**: Limit/Override moved INSIDE the discount panel; nothing dropped.
...
- Mobile action-bar tidy-wrap: `.tn-action-row1/.tn-action-row2 > button` stretch `flex:1 1 auto` + centered, order-type group stretches too — mobile.css §37 44px inflation otherwise wraps pills raggedly. Divider `div.w-px` and delivery-charge div deliberately NOT stretched.
