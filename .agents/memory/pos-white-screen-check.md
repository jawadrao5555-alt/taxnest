...
- `scripts/pos-white-screen-check.sh` runs automatically as a deploy-live.sh preflight (bypass only via `SKIP_WHITE_SCREEN_CHECK=1`). Two layers:
  ...
  2. **Runtime**: curl-logs-in as posadmin@taxnest.com, fetches 7 key POS pages, requires HTTP 200 + a language-independent marker (element ids / field names, NEVER translated text) + `node --check` passing on EVERY inline `<script>` of the rendered HTML.
...
- **How to apply:** when adding a critical POS page, add a `"/path|markerRegex"` entry to the PAGES array. Runtime part needs Laravel Server + MySQL Staging workflows up (exit 2 = couldn't run, distinct from exit 1 = broken page).
