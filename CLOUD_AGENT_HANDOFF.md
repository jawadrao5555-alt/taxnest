...
4. After an explicit owner approval phrase (`Approved — Merge & Deploy`, `Deploy kar do`, `Live kar do`, or `Approved, put it live`), the agent re-verifies the PR (Ready, targets `main`, required checks green, HEAD SHA unchanged, mergeable), shows the exact PR number + HEAD SHA, and runs `scripts/owner-merge-and-deploy-request.sh`. That script gates confirm on **origin/main** `decide()` (Actions checks out main, not the PR). If origin/main rejects an alias that only exists on the PR, use the phrase it currently accepts. Then it attempts `workflow_dispatch` of `.github/workflows/owner-merge-and-deploy.yml`. If `gh` is HTTP 403, print the Actions UI fill-ins; the owner clicks **Run workflow**. How-to: `docs/ops/owner-merge-and-deploy.md`. Agents must **not** `gh pr merge` or use the GitHub PR Merge button.
