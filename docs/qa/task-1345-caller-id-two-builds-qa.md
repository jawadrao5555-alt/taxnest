...
**Live par go-live ho chuka (20 Aug 2026):**

- [x] PHP deploy (`scripts/deploy-live.sh`) — live HEAD = workspace HEAD,
      caches + OPcache reset, live screen smoke PASS
- [x] `caller_app_latest_version = 1.1.0`, `caller_app_plus_latest_version = 1.1.0`
- [x] Elaan (What's New) row **#181**, audience `pos`, 6 points Roman Urdu
- [x] `/download` par card: `Android 8+ · APK · 4.4 MB · v1.1.0`, neeche
      *"SIM calls only. WhatsApp calls bhi chahiyen?"* aur WhatsApp wala hissa
      apne Play Protect qadam + plus APK ke button ke sath mojood
- [x] Version endpoints live par:

      /api/app-version?app=caller        → 1.1.0, taxnest-caller.apk
      /api/app-version?app=caller_plus   → 1.1.0, taxnest-caller-plus.apk
      /api/caller-app/v1/version?build=sim  → sim,  taxnest-caller.apk
      /api/caller-app/v1/version?build=plus → plus, taxnest-caller-plus.apk
      /api/caller-app/v1/version (no param) → plus  ← purane phone yahin girte hain

**Abhi baqi (sirf owner kar sakta hai):** asli phone par section 3 wali
checklist — yeh cheez kisi server test se prove nahi hoti.

**Rollback (agar phone test par kuch ghalat mile):** live par
`cd public_html/public/downloads && cp -f taxnest-caller-1.0.0.apk taxnest-caller.apk`,
aur admin settings mein dono version fields khali kar dein (khali = dono cards
chhup jate hain, koi update banner nahi).
...
## 4. Elaan jo POS users ko gaya (row #181, audience `pos`)

**Title:** Caller ID app ab bina rukawat ke install hoti hai

1. Google ki nai security (Play Protect) ki wajah se Caller ID app install hi
   nahi ho rahi thi — *"App blocked to protect your device"* aa raha tha. Ab yeh
   masla hal ho gaya hai.
2. Ab do version hain. Aam (default) app sirf SIM/dialer par aane wali calls
   pakadti hai aur bilkul seedhi install ho jati hai — koi warning nahi.
3. Jin shops ko WhatsApp calls bhi chahiyen, unke liye download page par alag
   "WhatsApp wali" APK hai, aur uske sath Play Protect band/chaalu karne ke qadam
   Roman Urdu mein likhe hain.
4. Sale screen ka popup pehle jaisa hi hai: grahak ka naam, khata balance, kitni
   dafa aaya, pichla order aur repeat-order ka button.
5. Purani app upar hi update ho jati hai — uninstall karne ki zaroorat nahi, aur
   WhatsApp wali app kabhi khud-ba-khud aam wali par down-grade nahi hogi.
6. Caller ID pehle ki tarah sirf Unlimited package walon ko milta hai aur
   POS → Customize se ON karna parta hai.

Agli release ka poora tareeqa (build → verify → host → settings → elaan)
`caller-app/RELEASE.md` mein hai.
