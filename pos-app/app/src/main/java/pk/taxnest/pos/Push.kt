...
/**
 * FCM token plumbing for the POS shell (v1.1.0, Task #1142) — instant
 * "naya order / order tayyar / day-close" push for waiter, cashier and owner.
 *
 * Mirrors the rider app's Fcm.kt degradation contract: EVERY call is wrapped
 * so the whole feature is a silent no-op when Firebase is unavailable
 * (build without google-services.json → FirebaseApp never initializes →
 * getInstance() throws, caught here; phone without Play Services → token
 * fetch just fails). The web app's normal polling/refresh keeps working —
 * push is additive, never a dependency.
 *
 * Unlike the rider app there is NO bearer token: the shell rides the
 * WebView's session cookie (CookieManager) for registration, plus the
 * X-TaxNest-App header the server requires as its cross-site-forgery guard.
 * Logout-time clear is stateless (authenticated by the token itself) because
 * the session cookie is already dead when the shell sees /pos/login again.
 */
object Push {
    ...
    private val REGISTER_URL = "https://" + MainActivity.BASE_HOST + "/pos/app/fcm-token"
    private val CLEAR_URL = "https://" + MainActivity.BASE_HOST + "/api/pos-app/fcm-token/clear"

    /**
     * Called from onPageStarted for every main-frame navigation. Login is
     * detected by landing on a first-party /pos/ page PAST the guest pages;
     * logout by landing back on /pos/login while we thought we were in.
     */
    fun onNavigation(c: Context, url: String) {
        try {
            ...
            val guest = path == "/pos" || path == "/pos/" ||
                path.startsWith("/pos/login") || path.startsWith("/pos/register") ||
                path.startsWith("/pos/invoice/share") // public receipt page — no session
            if (path.startsWith("/pos/") && !guest) {
                onLoggedIn(c)
            } else if (path.startsWith("/pos/login") && isLoggedIn(c)) {
                onLoggedOut(c)
            }
        } catch (e: Exception) {
    ...
    private fun postJson(url: String, body: JSONObject, withSession: Boolean): Int {
        ...
        return try {
            ...
            // Server's CSRF stand-in — only the shell sets this header.
            conn.setRequestProperty("X-TaxNest-App", "pos")
            if (withSession) {
