...
- **Restaurant POS Module:** Full restaurant management integrated into NestPOS at `/pos/restaurant/*`.
    ...
    - **Unified Top Nav Layout (No Sidebar):** All POS types use the same layout (`pos-app.blade.php`). Sidebar fully removed. Top nav bar (indigo/purple gradient) with: NestPOS logo (left), nav pills — Home, POS (restaurant only), New Sale, Orders, Tables/KDS (restaurant only) — (center), profile avatar dropdown (right). Profile dropdown contains all navigation: Products, Customers, Reports, Tax Reports, Day Close, Inventory (if enabled), Restaurant items (if restaurant mode), Admin settings (Services, Terminals, Team, Business Profile, PRA Settings, Billing), My Profile, Sign Out. Mobile: hamburger menu expands nav pills.
