...
- **Confidential PIN System:** Company admin sets 4-6 digit PIN in PRA Settings (bcrypt-hashed in `companies.confidential_pin`). PIN required to access local data tabs. PIN session expires after 30 minutes. 5 wrong attempts = 15 min lockout (cache-based). Server-side enforcement via `requirePinForLocalTab()` helper.
...
- **Role-based Sidebar Navigation:** Cashiers see limited sidebar: Dashboard, New Sale, Transactions, Reports, Tax Reports, Products (view), My Profile. Admin sees full sidebar including Services, Terminals, Customers, Team, Business Profile, PRA Settings, Billing.
