...
-   **Restaurant POS Print Idempotency + Last-Sale Reprint Widget:** Hard fix for production "KOT 2 dafa back-to-back, receipt nahi ati" + "Esc se reprint option ghaayb" bugs. (1) `printReceipt(onAfterPrint, force)` and `printKitchenTicket(orderId, onAfterPrint, force)` now carry an idempotency lock — same target within 5000ms is silently no-op'd (chain still advances via 50ms onAfterPrint timer). New state: `_lastReceiptPrintTxnId/_At`, `_lastKotPrintOrderId/_At`, `_printDedupMs`. New 3rd `force` param bypasses dedup for explicit cashier intent (success-modal Print/KOT buttons, P/K keyboard shortcuts, floating widget reprint actions). Auto-chain calls remain unforced so any double-fire (browser onload race, stale setTimeout, network retry) suppresses the duplicate. (2) Floating "Last Sale" reprint widget (fixed bottom-right, gated on `showLastSaleWidget && !showReceipt && lastTransactionId`) — three buttons: Receipt / KOT / View (re-opens success modal) + dismiss X. Set true in `payHeldOrderDirect` after successful pay, cleared in `startNewAfterPayment()`, `newSale()`, `voidOrder()` (NOT in `clearCart()` since post-pay path calls `clearCart` AFTER setting widget true). New methods: `reprintLastReceipt`, `reprintLastKot`, `reopenReceiptModal`, `dismissLastSaleWidget`. Console.log on each print fire for production debug visibility. Architect verdict: PASS.
