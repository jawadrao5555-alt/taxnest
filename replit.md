...
- **Restaurant POS Module:** Full restaurant management integrated into NestPOS at `/pos/restaurant/*`.
    ...
    - **EXE-Style Dashboard:** Desktop app look with indigo title bar (NestPOS branding, system status indicator, date/time), left sidebar navigation (Dashboard, POS, Orders, Tables, KDS, Menu, Inventory), gradient stat cards with animations, progress bars, glassmorphism cards, professional charts with tooltips, transaction table with hover effects, slide-in animations.
