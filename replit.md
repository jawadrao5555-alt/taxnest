...
- "Show Tax on Receipt" toggle OFF hides Subtotal + Tax on ALL POS receipts including PRA fiscal — customer copy shows grand TOTAL only (owner decision Jul 2026). Tax is ALWAYS submitted to PRA in full (details via Sahulat QR). Do NOT re-add a fiscal-always-show override. Toggle lives on /pos/receipt-settings, NOT POS Features.
...
- **Standalone POS Edition**: `companies.pos_integration_mode` 'pra' | 'standalone'. Standalone = full NestPOS, ZERO government integration, cheaper annual plan set (`pricing_plans.product_type='standalone'`: Trial 0 / Starter 2,999 / Business 4,999 / Pro 8,999). Registration has 2-card edition picker; all mode reads null-safe (`?? 'pra'`). Standalone→PRA flip is ONE-WAY, admin-only; sale-screen PRA toggle hidden + `togglePra` rejects standalone (422). Keeps `companies.product_type='pos'` so usage limits work unchanged.
