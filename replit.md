...
-   **Restaurant POS Compact Success Popup + Auto-Close-After-Print:** Replaced the full-page "receipt module" success modal (which embedded an iframe receipt preview taking ~45vh of screen) with a clean compact popup card (`max-w-sm`). New top-right cross button alongside Esc and click-outside for closing. Critical UX change: when ANY print command fires (auto-print on payment, manual Print/KOT button click, or `P`/`K` keyboard shortcut), the popup auto-closes AFTER the browser print dialog dismisses (via the existing `_printViaIframe` postMessage signal — `onAfterPrint` callback now sets `showReceipt = false`). Auto-print chain: `wantsAutoReceipt && wantsAutoKot` → receipt prints → receipt dialog closes → KOT prints → KOT dialog closes → popup closes. `wantsAutoReceipt` only → receipt prints → dialog closes → popup closes. If neither auto-print is on, popup stays open until user manually closes. The floating "Last Sale" reprint widget remains visible after popup closes for any subsequent reprint needs. KOT button on popup still gated by `kitchenSettings.print_on_hold` (only shown when auto-KOT is OFF, since auto-KOT-ON makes the manual button redundant). Also added `x-cloak` to ALL major modals (Pay/Table/Held/NewCustomer/CustomerPicker/ManagerPin/CustomerHistory/Shortcuts/LowStock/LastSaleWidget/SuccessPopup) to fix FOUC flash on initial page load.
