...
- **POS Cart Phase 4 — Restaurant POS Delete-Bug Fix (Cart `:key` Identity Crisis):** Fixed user-reported bug where deleting one cart item visually removed the WRONG (adjacent) item in `pos/restaurant/pos.blade.php`. **Root cause:** Restaurant POS used `:key="index"` in cart `<template x-for>` (universal POS already correctly used `:key="item.cart_uid"`). With `:key="index"`, when splicing item at index N, Alpine kept DOM[N] and re-bound it to the next item's data, but removed DOM[N+1] — so the LAST/NEXT row visually disappeared even though the data correctly removed the clicked item. **Compounded by** `removeFromCart` using `setTimeout(250ms)` for exit-animation — the captured `index` became stale if user mutated cart again during the 250ms window. **Fix (6 atomic edits):** (a) Line 375 `:key="index"` → `:key="item.cart_uid"`. (b) `addToCart` push (line 1298): added `cart_uid: 'c' + Date.now() + '_' + Math.random().toString(36).slice(2,9)` to push object. (c) `recallOrder.map` (line 1847): added cart_uid to mapped order items. (d) `reorderItems` push (line 1917): added cart_uid to favorite/history reorder push. (e) `restoreCart` localStorage (line 1187): backfill `cart_uid: i.cart_uid || (...)` so legacy cached carts also work. (f) `removeFromCart` (line 1324-1344): capture `uid` BEFORE setTimeout, then `findIndex(c => c.cart_uid === uid)` at splice time — eliminates stale-index race during 250ms exit animation. Brought Restaurant POS to full parity with Universal POS (which already had all 6 patterns correct). **Affected files:** `resources/views/pos/restaurant/pos.blade.php` only. Universal POS untouched (already correct). PRA POS variant inventory: `pos/universal.blade.php` (Retail — Phase 3 ✅, no delete bug), `pos/restaurant/pos.blade.php` (Dine-in — Phase 3 ✅ + Phase 4 ✅), `pos/create-invoice.blade.php` + `pos/edit-transaction.blade.php` (form-based — different code path, no Phase 3/4 needed), `fbr-pos/create.blade.php` (separate FBR module — different code path).
