...
- **Enterprise FBR Idempotency Shield (6-Phase):** (1) DB hard lock via `fbr_submission_hash` unique column + `request_payload_hash` indexed on fbr_logs. (2) Service-level Exception thrown for any duplicate/locked/pending state. (3) `DB::transaction` + `lockForUpdate()` for atomic submission with race-condition double-check. (4) Rate format enforced via `rtrim+append %`. (5) Tinker blocked in production via AppServiceProvider. (6) Duplicate payload hash detection against successful fbr_logs. Hash cleared on any failure to allow retry.
- **Enterprise UX Simplification (Feb 2026):** Invoice lifecycle reduced to 3 states only: `draft`, `locked`, `pending_verification`. The `submitted` and `failed` statuses are completely removed. On FBR submit: status stays `draft` with `is_fbr_processing=true` flag. On FBR success: `status=locked, fbr_status=production`. On failure: `status=draft, fbr_status=failed`. On ambiguous: `status=pending_verification`. The `is_fbr_processing` boolean column replaces the old `submitted` status as a temporary processing flag. Guards check: locked, pending_verification, is_fbr_processing, existing fbr_invoice_number, and successful fbr_logs.
