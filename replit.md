...
- **Restaurant POS Module:** Full restaurant management integrated into NestPOS at `/pos/restaurant/*`. Includes: Restaurant POS Screen (Aronium-style with product grid + order panel), Table Management (floors + tables with status tracking), Kitchen Display System (KDS with real-time polling, order status transitions: held→preparing→ready→completed), Ingredient Inventory (raw materials with stock levels, low-stock alerts, stock adjustments), Recipe/BOM Engine (link ingredients to products, auto-deduct on payment), Customer CRM (search + quick-add). Orders flow: Hold→KDS→Pay. Inventory deducts ONLY on Pay. Tables: `restaurant_floors`, `restaurant_tables`, `restaurant_orders`, `restaurant_order_items`, `ingredients`, `product_recipes`. Controllers: `RestaurantPosController`, `RestaurantTableController`, `RestaurantKdsController`, `IngredientController`.
