...
- TABLE BOARD (Jul 2026, naya): Table Management ON ho to sale screen par cart ke NEECHE tables ka board hamesha nazar aata hai — har table ka rang (sabz = khali, amber = reserved, laal = order chal raha hai, jamni = waiter ka order tayyar), kitni der se order laga hai, kis staff ka order hai aur bill ki raqam. Table par CLICK karne se chhota MENU khulta hai (seedha koi action nahi hota): "View / Edit" (order cart mein kholein), "FINAL karo" (pehle CASH ya CARD ki tasdeeq poochta hai — ghalti se bill final hone ka masla khatam), "KOT Dobara Bhejo", aur "Order Cancel + Table Khali" (waiter ke orders cashier cancel nahi kar sakta). Khali table par click karke Reserve bhi kar sakte hain. Board khud har ~25 second mein taaza hota hai; upar wale teer se board ko band/khol sakte hain (keyboard se Alt+B bhi). F3 wala table picker pehle jaisa hi hai.
- TABLE BOARD ki behtariyan (Jul 2026 update): (1) DER wale orders khud UJAGAR hote hain — waiter ka order ~10 minute tak koi na uthaye, table par order ~30 minute se chal raha ho, ya reservation ~15 minute purani ho to us tile ka rang GEHRA ho jata hai, ⚠ ka nishaan aata hai aur waqt wali chip blink karti hai; (2) board ke upar "Rs X chalu" — is waqt tamam khule orders ki KUL raqam nazar aati hai; (3) COUNTER ORDERS: waiter ke bina-table orders (takeaway/delivery) bhi board par alag chips ban kar aate hain — click karke seedha khol kar bill kar sakte hain; jamni badge mein inki ginti bhi shamil hoti hai.
