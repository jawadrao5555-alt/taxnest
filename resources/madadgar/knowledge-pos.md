...
- Multi-word search (Aug 2026): ek se zyada lafz likhein to har lafz naam ke kisi lafz se match hota hai — misal ke liye "{example}" ke naam ke alfaz likhein. Bracket wale sizes bhi lafz ki tarah milte hain. Pehla lafz phir bhi naam ke shuru se hona chahiye (Any-word mode mein yeh bhi zaroori nahi). Yehi rule waiter screen par bhi hai.
...
- Quick Type Mode (F7): "{quick_type}" jaisi line likhein — pura order khud cart mein aa jata hai. DEFAULT BAND hai; admin /pos/customize se ON kare.
...
- Hold (F5): Dine-In, Takeaway aur Delivery teenon order hold karein — Table Management ON ho to Dine-In ke liye table zaroori hai. Baad mein TABLE board (Alt+B ya TABLE button) se wapas kholein: table wale orders table card par, baqi "Held Orders (bina table)" amber chips mein. Manual items aur deals hold NAHI ho sakte (sirf seedha bill). (Table Management OFF walon ke liye Held ki purani pill upar top bar mein hi hai.)
...
- "Send to Kitchen" (KOT) button: Dine-In, Takeaway ya Delivery order ko bina payment ke kitchen bhejta hai aur kitchen ticket print hota hai. Table Management ON ho to Dine-In ke liye table zaroori hai.
...
- KHULE ORDERS ke sath din band NAHI hota (10 Aug 2026 se): agar tables/takeaway/delivery ke orders abhi khule hon to "Close Day" ruk jata hai — pehle har khula order final karein ya cancel karein, phir din band hoga. (Pehle "phir bhi band karo" ka option tha — ab hata diya gaya hai taake khule orders ghalti se urr na jayen.)
...
- Recipes / Ingredients: /pos/products par product ke saath recipe usi Add/Edit form mein manage ho sakti hai. Tafseeli alag workflow ke liye /pos/restaurant/ingredients par kachha maal (Name, Unit: kg/g/ltr/ml/pcs/dozen/pack, Cost/Unit, Opening Stock, Min Stock) → /pos/restaurant/recipes par product chunein aur ingredient + Qty Needed rows dalein → Save Recipe. Dish bikne par ingredients ka stock khud katta hai. Ingredients ka stock adjust bhi ingredients page par hota hai.
...
- DELIVERY BOARD sale screen par (10 Aug 2026 se): sale screen ke upar GREEN "Deliveries" button dabane se Delivery Board ek popup mein khul jata hai — rider assign, Delivered mark aur rider ka cash Settle sab isi se ho jata hai, /pos/deliveries page kholne ya alag "Delivery Manager" ID se login karne ki zaroorat nahi. (AMBER "Pending Deliveries" button alag cheez hai — wo sirf pending delivery bills ko Cash/Card par final karta hai.)
- Rider assign: bill BANNE KE BAAD — sale screen ke GREEN "Deliveries" (Delivery Board) se ya /pos/deliveries board par "Assign Rider" dropdown se (payment modal mein assign nahi hota).
...
- Cash settle: /pos/deliveries par (ya sale screen ke Delivery Board mein) rider ke card par "Settle Cash" → jo bills settle ho rahe hain tick karein → note (optional) → "Confirm Settlement". Partial settlement ho sakta hai.
...
- "{item} search mein nahi mil raha": search naam ke SHURU se chalti hai — PEHLA lafz naam ke shuru se likhein (maslan "{example}" ke naam ka pehla lafz). Ek se zyada lafz likh sakte hain (Aug 2026): baqi alfaz naam ke kisi bhi lafz se mil jate hain; bracket wale sizes bhi milte hain. Barcode/SKU poora ya digits ke saath likhein to woh bhi mil jata hai. Phir bhi na mile to /pos/products par check karein — {item} inactive to nahi? Search har category mein dhoondti hai.
...
- "Hold nahi ho raha": Dine-In, Takeaway aur Delivery teenon hold ho sakte hain; Table Management ON ho to Dine-In table ke baghair hold nahi hoga, aur manual items/deals hold nahi ho sakte.
...
## Day close aur pending provisional bills — Carry Forward / Khud Final (Aug 2026)
- Sawal: "Auto day-close (misal 6 baje) par jo bills Make Final karna bhool gaye unka kya?" — Customize POS → Local Billing mein provisional bills ki policy ab 4 options: Archive (mehfooz magar F10 se ghayab), Delete (urh jate hain), **Carry (Agle Din)** — bills F10 mein rahte hain aur agle din Make Final ho sakte hain — ya naya **Khud Final**. Raat-batch delivery walon ke liye Carry ya Khud Final best hai.
- **Khud Final (Aug 2026):** day close (manual ya 6 baje auto) par bache hue provisional bills KHUD Make Final ho jate hain — bilkul wahi tarika jo F10 Make Final use karta hai: quota katta hai, reporting ON ho to PRA ko submit hota hai (internet/PRA down ho to bill Offline mein ja kar dobara try hota hai), reporting OFF ho to aam final bill banta hai. Receipt print nahi hoti (customer maujood nahi). Purane mahine ke bills aur quota khatam hone par bache bills Carry ho jate hain (agle din tak pending). KHABARDAR: ghalat/test bills bhi tax record mein chale jayenge aur PRA submit wapas nahi hota — is liye yeh option soch samajh kar ON karein.
- Day-close page ab pending provisional bills par surkh warning dikhata hai (agar policy Carry ya Khud Final nahi) — pehle F10 se final karein.
...
- Waiter ke TAKEAWAY/DELIVERY orders (10 Aug 2026 se): ghanti (incoming orders) wale panel mein har order par Cancel ka option hai — wahin se cancel karein (yeh orders table board par nahi aate). Cancel hua order Cancelled Orders report mein chala jata hai.
