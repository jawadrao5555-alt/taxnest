<x-admin-layout>
                                ...
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-900/30">
                                    ...
                                    <td class="px-4 py-3">
                                        <div class="flex items-center gap-1.5 flex-wrap">
                                            <button type="button"
                                                onclick='openEditModal(@json($upd->id), @json($upd->title), @json(implode("\n", $upd->points ?? [])), @json($upd->image_path ? asset("storage/" . $upd->image_path) : null), @json($upd->audience), @json((bool) ($upd->is_featured ?? false)), @json($upd->type), @json($upd->audience_family))'
    ...
    <script>
        function openEditModal(id, title, pointsText, imageUrl, audience, isFeatured, type, audienceFamily) {
