<x-admin-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="font-bold text-xl text-gray-800 dark:text-gray-100 leading-tight">Audit Logs</h2>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="bg-white dark:bg-gray-900 rounded-xl shadow-sm border border-gray-200 dark:border-gray-800 p-4 mb-6">
                <form method="GET" action="{{ route('admin.audit-logs') }}" class="flex items-center gap-4">
                    <div>
                        <label class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Filter by Action</label>
                    ...
                    <div class="mt-5">
                        ...
                        <a href="{{ route('admin.audit-logs') }}" class="ml-2 text-sm text-gray-500 hover:text-gray-700 dark:text-gray-300">Clear</a>
                        @endif
                    </div>
                </form>
            </div>
                ...
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    <thead class="bg-gray-50 dark:bg-gray-800">
                        <tr>
                            ...
                            <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">Entity Type</th>
                            <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">Entity ID</th>
                            <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">SHA256 Hash</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                        @forelse($logs as $log)
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-800 dark:bg-gray-800">
                            <td class="px-4 py-3 text-sm text-gray-700 dark:text-gray-300 whitespace-nowrap">{{ $log->created_at?->format('d M Y H:i') ?? '-' }}</td>
                            <td class="px-4 py-3 text-sm text-gray-700 dark:text-gray-300">{{ $log->company->name ?? 'System' }}</td>
                            ...
                            <td class="px-4 py-3 text-sm text-gray-600 dark:text-gray-400">{{ $log->entity_type ?? '-' }}</td>
                            <td class="px-4 py-3 text-sm font-mono text-gray-600 dark:text-gray-400">{{ $log->entity_id ?? '-' }}</td>
