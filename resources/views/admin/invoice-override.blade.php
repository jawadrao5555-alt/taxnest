<x-admin-layout>
    <x-slot name="header">
        <h2 class="font-bold text-xl text-gray-800 dark:text-gray-100 leading-tight">Invoice Override</h2>
    </x-slot>
    ...
    <div class="py-8">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="bg-white dark:bg-gray-900 rounded-xl shadow-sm border border-gray-200 dark:border-gray-800 p-6 mb-6">
                <h3 class="text-lg font-semibold text-gray-800 dark:text-gray-100 mb-4">Search Invoices</h3>
                <form method="GET" action="/admin/invoice-override" class="flex gap-4">
                    <input type="text" name="q" value="{{ request('q') }}" placeholder="Search by invoice number, company name, or invoice ID..." class="flex-1 rounded-lg border-gray-300 dark:border-gray-700 dark:bg-gray-800 dark:text-white shadow-sm focus:ring-emerald-500 focus:border-emerald-500">
            ...
            <div class="bg-white dark:bg-gray-900 rounded-xl shadow-sm border border-gray-200 dark:border-gray-800 overflow-hidden">
                <div class="px-6 py-4 border-b border-gray-100 dark:border-gray-800">
                    <h3 class="text-lg font-semibold text-gray-800 dark:text-gray-100">Results for "{{ request('q') }}" <span class="text-sm font-normal text-gray-500">({{ $invoices->count() }} found)</span></h3>
                </div>
                    ...
                    <table class="w-full text-sm">
                        <thead>
                            <tr class="bg-gray-50 dark:bg-gray-800 border-b border-gray-100 dark:border-gray-800">
                                ...
                                <th class="text-left py-3 px-4 font-semibold text-gray-600 dark:text-gray-400">Company</th>
                                <th class="text-right py-3 px-4 font-semibold text-gray-600 dark:text-gray-400">Amount</th>
                                <th class="text-center py-3 px-4 font-semibold text-gray-600 dark:text-gray-400">Status</th>
                                <th class="text-center py-3 px-4 font-semibold text-gray-600 dark:text-gray-400">FBR Status</th>
                                <th class="text-left py-3 px-4 font-semibold text-gray-600 dark:text-gray-400">FBR Invoice #</th>
                        ...
                        <tbody>
                            ...
                            <tr class="border-b border-gray-50 hover:bg-gray-50/50" id="invoice-row-{{ $invoice->id }}">
                                ...
                                <td class="py-3 px-4 text-gray-600 dark:text-gray-400">{{ $invoice->company->name ?? 'N/A' }}</td>
                                <td class="py-3 px-4 text-right font-medium text-gray-800 dark:text-gray-100">{{ number_format($invoice->total_amount, 2) }}</td>
                                <td class="py-3 px-4 text-center">
                                    <span id="status-badge-{{ $invoice->id }}" class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium
                                        ...
                                        @elseif($invoice->status === 'draft') bg-gray-100 text-gray-800 dark:text-gray-100
                                ...
                                <td class="py-3 px-4 text-center">
                                    <span id="fbr-status-badge-{{ $invoice->id }}" class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium
                                        ...
                                        @else bg-gray-100 text-gray-500 dark:text-gray-400
                                        @endif">
                                        {{ $invoice->fbr_status ? ucfirst(str_replace('_', ' ', $invoice->fbr_status)) : 'None' }}
                                    </span>
                                </td>
                                <td class="py-3 px-4 text-gray-600 dark:text-gray-400">{{ $invoice->fbr_invoice_number ?? '-' }}</td>
                                <td class="py-3 px-4 text-center">
        ...
        function updateBadges(invoiceId, data) {
            ...
            if (statusBadge) {
                const statusColors = {
                    ...
                    'draft': 'bg-gray-100 text-gray-800 dark:text-gray-100',
            ...
            if (fbrBadge && data.fbr_status !== undefined) {
                ...
                fbrBadge.className = 'inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ' +
                    (fbrColors[data.fbr_status] || 'bg-gray-100 text-gray-500 dark:text-gray-400');
