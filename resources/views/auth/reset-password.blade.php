<!DOCTYPE html>
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}" {!! app()->getLocale() === 'ur' ? 'dir="rtl"' : '' !!}>
<head>
    <meta charset="UTF-8">
    ...
    <div class="w-full max-w-md mx-4">
        ...
        <div class="rounded-2xl p-8" style="background: rgba(255,255,255,0.06); border: 1px solid rgba(255,255,255,0.1); backdrop-filter: blur(20px);">
            <div class="text-center mb-6">
                ...
                <h2 class="text-xl font-bold text-white">{{ __('pos.auth_rp_heading') }}</h2>
                <p class="text-sm text-emerald-200/50 mt-2">{{ __('pos.auth_rp_sub') }} <span class="text-emerald-300 font-medium">{{ $email }}</span></p>
            </div>
            ...
            <form method="POST" action="{{ route('password.store') }}" class="space-y-4">
                ...
                <div>
                    <label class="block text-sm font-medium text-emerald-100/70 mb-1.5">{{ __('pos.auth_confirm_password') }}</label>
                    <input type="password" name="password_confirmation" required autocomplete="new-password" class="w-full px-4 py-2.5 rounded-xl text-sm text-white placeholder-emerald-300/30 transition" style="background: rgba(255,255,255,0.08); border: 1px solid rgba(255,255,255,0.12); outline: none;" placeholder="{{ __('pos.auth_ph_reenter') }}" onfocus="this.style.borderColor='rgba(52,211,153,0.5)'; this.style.boxShadow='0 0 0 3px rgba(52,211,153,0.12)';" onblur="this.style.borderColor='rgba(255,255,255,0.12)'; this.style.boxShadow='none';">
