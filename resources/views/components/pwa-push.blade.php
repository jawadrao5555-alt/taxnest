    ...
    async function maybeAskPermission(){
        ...
        const askOnce = async () => {
            ...
            const ok = await new Promise(resolve => {
                ...
                bar.innerHTML = '<div style="font-weight:700;margin-bottom:4px">' + @json(__('pos.pwa_push_title')) + '</div><div style="opacity:.85;margin-bottom:10px">' + @json(__('pos.pwa_push_body')) + '</div><div style="display:flex;gap:8px;justify-content:flex-end"><button id="tnPushNo" style="background:#374151;color:#fff;border:0;padding:6px 12px;border-radius:6px;cursor:pointer;font-size:12px">' + @json(__('pos.pwa_push_not_now')) + '</button><button id="tnPushYes" style="background:#10b981;color:#fff;border:0;padding:6px 14px;border-radius:6px;cursor:pointer;font-weight:600;font-size:12px">' + @json(__('pos.pwa_push_enable')) + '</button></div>';
