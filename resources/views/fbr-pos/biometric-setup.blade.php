...
<div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
    ...
    {{-- ═══ Biometric Device Setup — FBR panel port (Aug 2026) ═══
         Admin-only: register ZKTeco/compatible devices + map device PINs
         to FBR team members. Token-based ADMS push endpoint is SHARED with
         the PRA panel (company-scoped by token/serial) — only this page,
         its routes and guard are FBR-specific. Blue accent (FBR family)
         instead of the PRA purple. --}}
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6">
        ...
        <div class="flex gap-2">
            <a href="{{ route('fbrpos.bio-sync.import') }}" class="inline-flex items-center gap-2 px-4 py-2 bg-emerald-600 text-white text-sm font-semibold rounded-lg hover:bg-emerald-700 transition">
            ...
            <button onclick="document.getElementById('add-device-form').classList.toggle('hidden')" class="inline-flex items-center gap-2 px-4 py-2 bg-blue-600 text-white text-sm font-semibold rounded-lg hover:bg-blue-700 transition">
    ...
    <div id="add-device-form" class="hidden mb-6 bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm p-5">
        ...
        <form method="POST" action="{{ route('fbrpos.bio-sync.store-device') }}">
            @csrf
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="label" required maxlength="100" placeholder="{{ __('pos.bio_device_label_ph') }}"
                           class="w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 text-sm px-3 py-2 focus:ring-2 focus:ring-blue-500 transition">
                </div>
                <div>
            ...
            <div class="mt-4 flex items-center gap-3">
                <button type="submit" class="px-5 py-2 bg-blue-600 text-white text-sm font-semibold rounded-lg hover:bg-blue-700 transition">{{ __('pos.save') }}</button>
    ...
    {{-- ── Late-arrival marking (Task #1274) ──────────────────────────────
         Admin sets a wall-clock threshold; staff whose FIRST check-in of a
         business day is after it get a red "late" chip on the hazri report
         and a Late-Days count in the payroll summary/PDF. Empty = off. --}}
    <div class="mb-6 bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm p-5">
        <div class="flex items-start justify-between gap-4 flex-wrap">
            <div class="min-w-0">
                <h2 class="text-base font-bold text-gray-800 dark:text-white flex items-center gap-2">
                    ...
                    {{ __('pos.bio_late_after_title') }}
                </h2>
                <p class="text-xs text-gray-500 dark:text-gray-400 mt-1 max-w-xl">{{ __('pos.bio_late_after_hint') }}</p>
            </div>
            <form method="POST" action="{{ route('fbrpos.bio-sync.save-late') }}" class="flex items-end gap-2">
                @csrf
                <input type="time" name="late_after" value="{{ old('late_after', $company->pos_bio_late_after ?? '') }}"
                       class="rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 text-sm px-3 py-2 focus:ring-2 focus:ring-blue-500 transition">
                <button type="submit" class="px-4 py-2 bg-blue-600 text-white text-sm font-semibold rounded-lg hover:bg-blue-700 transition">{{ __('pos.save') }}</button>
            </form>
        </div>
    </div>
    ...
    <div class="mb-6 bg-amber-50 dark:bg-amber-900/20 border border-amber-300 dark:border-amber-700 rounded-xl overflow-hidden">
        ...
        <div class="divide-y divide-amber-100 dark:divide-amber-800/40">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center gap-3 px-4 py-3">
                ...
                <form method="POST" action="{{ route('fbrpos.bio-sync.quick-map') }}" class="flex items-center gap-2">
                    @csrf
    ...
    <div class="mb-6 bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden">
        ...
        <div class="px-5 py-4 bg-gray-50 dark:bg-gray-800 border-b border-gray-100 dark:border-gray-700 flex items-center justify-between gap-3 flex-wrap">
            <div class="flex items-center gap-3">
                <div class="w-9 h-9 rounded-lg bg-blue-100 dark:bg-blue-900/30 flex items-center justify-center">
                    <svg class="w-5 h-5 text-blue-600 dark:text-blue-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 3H5a2 2 0 00-2 2v4m6-6h10a2 2 0 012 2v4M9 3v18m0 0h10a2 2 0 002-2V9M9 21H5a2 2 0 01-2-2V9m0 0h18"/></svg>
                </div>
                <div>
            ...
            <div class="flex items-center gap-2">
                ...
                @endif
                <form method="POST" action="{{ route('fbrpos.bio-sync.toggle-device', $device->id) }}" class="inline">
                    @csrf
                ...
                <form method="POST" action="{{ route('fbrpos.bio-sync.destroy-device', $device->id) }}" class="inline" onsubmit="return confirm({{ Js::from(__('pos.bio_confirm_delete')) }})">
                    @csrf @method('DELETE')
        ...
        <div class="px-5 py-3 border-b border-gray-100 dark:border-gray-700 bg-white dark:bg-gray-900">
            ...
            <div class="flex items-center gap-2">
                ...
                <button onclick="navigator.clipboard.writeText(document.getElementById('url-{{ $device->id }}').textContent.trim()); this.textContent='✓'" class="text-xs px-3 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition whitespace-nowrap">{{ __('pos.copy') }}</button>
            </div>
        ...
        <div class="px-5 py-4">
            ...
            <form method="POST" action="{{ route('fbrpos.bio-sync.save-mapping', $device->id) }}" x-data="bioMapping({{ Js::from($device->maps->map(fn($m) => ['pin' => $m->device_pin, 'user_id' => $m->user_id])) }})">
                @csrf
                <template x-for="(row, i) in rows" :key="i">
                    <div class="flex items-center gap-2 mb-2">
                        <input type="text" :name="'mappings[' + i + '][device_pin]'" x-model="row.pin" placeholder="{{ __('pos.bio_pin_ph') }}" maxlength="50"
                               class="w-28 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 text-sm px-3 py-2 focus:ring-2 focus:ring-blue-500 transition">
                        <span class="text-gray-400 text-xs">→</span>
                        <select :name="'mappings[' + i + '][user_id]'" x-model="row.user_id"
                                class="flex-1 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 text-sm px-3 py-2 focus:ring-2 focus:ring-blue-500 transition">
                            <option value="">— {{ __('pos.bio_select_user') }} —</option>
                            @foreach($posUsers as $u)
                            <option value="{{ $u->id }}">{{ $u->name }}</option>
                            @endforeach
                ...
                <div class="flex items-center gap-3 mt-3">
                    <button type="button" @click="rows.push({pin:'',user_id:''})" class="text-sm text-blue-600 dark:text-blue-400 hover:underline flex items-center gap-1">
                        <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/></svg>
                        {{ __('pos.bio_add_row') }}
                    </button>
