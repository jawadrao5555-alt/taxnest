...
<div class="fbr-pos-stage max-w-7xl mx-auto pb-32 px-3 sm:px-4" x-data="fbrPosInvoice()" @click="userActivity()"
    ...
    <div class="mb-4 flex flex-wrap items-center justify-between gap-3 bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-700 rounded-2xl p-3 shadow-sm">
        ...
        <div class="flex items-center gap-2">
            {{-- 🟢/🟡/🔴 Auto-Sync status pill (FBR POS) --}}
            <div class="flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl text-[11px] font-bold border transition"
                 :class="syncStatus === 'online' ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : (syncStatus === 'syncing' ? 'bg-amber-50 text-amber-700 border-amber-200' : 'bg-red-50 text-red-700 border-red-200')"
                ...
                <span x-show="failedBills.length > 0" class="ml-0.5 px-1.5 rounded-full text-[9px] font-black bg-red-600 text-white" x-text="failedBills.length"></span>
            </div>
    ...
    <form method="POST" action="{{ route('fbrpos.store') }}" x-ref="saleForm" novalidate
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 lg:gap-6 items-start">
            ...
            <aside class="lg:col-span-1 lg:order-1 space-y-3 lg:sticky lg:top-16 lg:self-start">
                ...
                {{-- ═════════ 💳 PAYMENT & DISCOUNT + TOTALS + CONFIRM (moved from right column) ═════════ --}}
                <div class="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 shadow-md p-4">
                    <h3 class="text-sm font-black text-slate-900 dark:text-white mb-3 flex items-center gap-2 tracking-tight">
                        <span class="inline-flex items-center justify-center w-6 h-6 rounded-md bg-indigo-600 text-white text-xs shadow-sm">💳</span>
                        Payment &amp; Discount
                    </h3>
                    <div class="space-y-2.5">
                        <div>
                            <label class="block text-[10px] font-black text-slate-700 dark:text-slate-200 mb-1 tracking-wide uppercase">Method *</label>
                            ...
                            <p class="text-[10px] font-semibold text-slate-600 dark:text-slate-300 mt-1">Press <kbd class="px-1 bg-gray-200 dark:bg-gray-700 rounded text-[10px]">F8</kbd> for payment</p>
                        </div>
                        <div>
