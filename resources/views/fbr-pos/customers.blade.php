<x-fbr-pos-layout>
{{-- Task 1260: FBR POS Customers page — port of resources/views/pos/customers.blade.php
     (PRA) into the FBR layout: blue accent instead of purple, fbrpos.* routes,
     saved-address panel hits the /fbr-pos/api/customer-addresses twins, and the
     PRA-only khamosh-repeat chip is dropped (that service reads PRA transactions). --}}
<div class="p-4 sm:p-6 max-w-7xl mx-auto">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6">
        ...
        <div class="flex flex-wrap items-center gap-2">
            @if(!($isCashier ?? false))
            <a href="{{ route('fbrpos.customers.export') }}" class="bg-emerald-600 hover:bg-emerald-700 text-white px-3 py-2 rounded-lg text-sm font-semibold shadow-md transition">{{ __('pos.export_csv') }}</a>
            ...
            @endif
            <button onclick="document.getElementById('addCustomerForm').classList.toggle('hidden')" class="bg-gradient-to-r from-blue-500 to-blue-700 text-white px-4 py-2 rounded-lg text-sm font-semibold shadow-md hover:shadow-lg transition">{{ __('pos.add_customer_btn') }}</button>
        </div>
    </div>
    ...
    <div id="importCustomerForm" class="hidden mb-6 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl shadow-md p-5">
        ...
        <form method="POST" action="{{ route('fbrpos.customers.import') }}" enctype="multipart/form-data" class="flex flex-col sm:flex-row sm:items-center gap-3">
            @csrf
            <input type="file" name="file" accept=".csv,text/csv" required class="text-sm text-gray-700 dark:text-gray-300 file:mr-3 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-blue-100 file:text-blue-700 hover:file:bg-blue-200">
            ...
            <a href="{{ route('fbrpos.customers.template') }}" class="text-xs text-blue-600 hover:text-blue-700 underline">{{ __('pos.download_template') }}</a>
        </form>
    </div>
    @endif
    ...
    <div id="addCustomerForm" class="hidden mb-6 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl shadow-md p-5">
        ...
        <form method="POST" action="{{ route('fbrpos.customers.store') }}" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
            @csrf
            <div>
                <label class="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">{{ __('pos.customer_name_label') }}</label>
                <input type="text" name="name" placeholder="{{ __('pos.ph_full_name_optional') }}" class="w-full text-sm rounded-lg border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-white px-3 py-2 focus:ring-2 focus:ring-blue-500">
            </div>
            <div>
                <label class="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">{{ __('pos.phone_label') }}</label>
                <input type="text" name="phone" placeholder="03XX-XXXXXXX" class="w-full text-sm rounded-lg border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-white px-3 py-2 focus:ring-2 focus:ring-blue-500">
            </div>
            <div>
                <label class="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">{{ __('pos.email_label') }}</label>
                <input type="email" name="email" placeholder="email@example.com" class="w-full text-sm rounded-lg border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-white px-3 py-2 focus:ring-2 focus:ring-blue-500">
            </div>
            <div>
                <label class="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">{{ __('pos.type_label') }} *</label>
                <select name="type" required class="w-full text-sm rounded-lg border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-white px-3 py-2 focus:ring-2 focus:ring-blue-500">
            ...
            <div>
                ...
                <input type="text" name="cnic" placeholder="XXXXX-XXXXXXX-X" class="w-full text-sm rounded-lg border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-white px-3 py-2 focus:ring-2 focus:ring-blue-500">
            </div>
            <div>
                <label class="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">{{ __('pos.ntn_label') }}</label>
                <input type="text" name="ntn" placeholder="{{ __('pos.ph_ntn_number') }}" class="w-full text-sm rounded-lg border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-white px-3 py-2 focus:ring-2 focus:ring-blue-500">
            </div>
            <div>
            ...
            <div class="sm:col-span-2 lg:col-span-4">
                ...
                <input type="text" name="address" placeholder="{{ __('pos.ph_full_address') }}" class="w-full text-sm rounded-lg border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-white px-3 py-2 focus:ring-2 focus:ring-blue-500">
            </div>
        </form>
    </div>

    {{-- Customer search — SERVER-SIDE (PRA ZFC lesson): the page is paginated,
         so the search must hit the DB. The client-side row filter below stays
         as an instant filter WITHIN the current page. --}}
    <div class="mb-3">
        <form method="GET" action="{{ route('fbrpos.customers') }}" class="flex items-center gap-2 max-w-xl">
            <div class="relative flex-1">
                ...
                <input type="text" id="custSearchInput" name="q" value="{{ $q ?? '' }}" placeholder="{{ __('pos.search_customer_placeholder') }}" autocomplete="off" data-lpignore="true" data-form-type="other" data-1p-ignore
                       class="w-full text-sm rounded-lg border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-white pl-9 pr-3 py-2 focus:ring-2 focus:ring-blue-500">
            </div>
            <button type="submit" class="px-4 py-2 text-sm font-bold text-white bg-blue-600 hover:bg-blue-700 rounded-lg">{{ __('pos.search_btn') }}</button>
            @if(($q ?? '') !== '')
            <a href="{{ route('fbrpos.customers') }}" class="px-3 py-2 text-sm font-bold text-gray-600 dark:text-gray-300 border border-gray-300 dark:border-gray-600 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-800">{{ __('pos.clear_btn') }}</a>
            @endif
        </form>
            ...
            <table class="w-full text-sm table-cards">
                <thead>
                    <tr class="text-left text-xs text-gray-500 uppercase border-b border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-800/50">
                        ...
                        {{-- PRA parity (21 Aug 2026): city column ab PATA dikhata hai,
                             sheher uske neeche chhoti line mein. --}}
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                    ...
                    <tr class="cust-row {{ $loop->even ? 'bg-gray-50/50 dark:bg-gray-800/20' : '' }} {{ !$customer->is_active ? 'opacity-50' : '' }}" x-data="custRow({{ (int) $customer->id }})"
                        ...
                        <td class="px-4 py-3 text-center hidden sm:table-cell">
                            @if(!($isCashier ?? false))
                            <form method="POST" action="{{ route('fbrpos.customers.toggle', $customer->id) }}" class="inline">
                                @csrf
                        ...
                        <td class="px-4 py-3 text-center">
                            <div class="flex items-center justify-center gap-1">
                                <a href="{{ route('fbrpos.customers.history', $customer->id) }}" class="text-xs text-emerald-600 hover:text-emerald-700 px-2 py-1">{{ __('pos.history_word') }}</a>
                                <button @click="editing = !editing" class="text-xs text-blue-600 hover:text-blue-700 px-2 py-1">{{ __('pos.edit') }}</button>
                                <form method="POST" action="{{ route('fbrpos.customers.delete', $customer->id) }}" onsubmit="return confirm({{ Js::from(__('pos.confirm_delete_customer')) }})" class="inline">
                                    @csrf @method('DELETE')
                    ...
                    @if(!($isCashier ?? false))
                    <tr x-show="editing" class="bg-blue-50/50 dark:bg-blue-900/10">
                        <td colspan="7" class="px-4 py-3">
                            <form method="POST" action="{{ route('fbrpos.customers.update', $customer->id) }}" class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2 items-end">
                                @csrf @method('PUT')
                                ...
                                <div class="flex gap-2 col-span-2 sm:col-span-1">
                                    <button type="submit" class="text-xs font-semibold text-white px-3 py-1.5 rounded-lg bg-blue-600 hover:bg-blue-700 transition">{{ __('pos.save_btn') }}</button>
                                    <button type="button" @click="editing = false" class="text-xs text-gray-500 px-3 py-1.5">{{ __('pos.cancel') }}</button>
                                </div>
                            </form>
                            {{-- Saved delivery addresses (pos_customer_addresses) — the FBR sale
                                 screen already shares these endpoints (fbrpos.api.customer-addresses
                                 → PosController handlers). Loaded lazily when the edit row opens. --}}
                            <div class="mt-3 pt-3 border-t border-blue-100 dark:border-blue-900/30">
                                ...
                                <ul class="space-y-1">
                                    <template x-for="a in addresses" :key="a.id">
                                        <li class="flex items-center gap-2 text-xs text-gray-700 dark:text-gray-300">
                                            <span x-show="a.id === 0" class="inline-flex px-1.5 py-0.5 rounded bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300 font-medium shrink-0">{{ __('pos.default_addr_label') }}</span>
                                ...
                                <div class="flex items-center gap-1.5 mt-2">
                                    <input type="text" x-model="newAddrLabel" maxlength="50" placeholder="{{ __('pos.ph_addr_label') }}" autocomplete="off" name="fbr_cust_addr_label_nofill" data-lpignore="true" data-form-type="other" data-1p-ignore class="w-28 shrink-0 text-xs rounded-md border-gray-200 dark:border-gray-700 dark:bg-gray-800 dark:text-white py-1.5 px-2 focus:ring-blue-500 focus:border-blue-400">
                                    <input type="text" x-model="newAddrText" maxlength="500" @keydown.enter.prevent="saveAddress()" placeholder="{{ __('pos.ph_full_delivery_address') }}" autocomplete="off" name="fbr_cust_addr_nofill" data-lpignore="true" data-form-type="other" data-1p-ignore class="flex-1 min-w-0 text-xs rounded-md border-gray-200 dark:border-gray-700 dark:bg-gray-800 dark:text-white py-1.5 px-2 focus:ring-blue-500 focus:border-blue-400">
                                    <button type="button" @click="saveAddress()" :disabled="addrSaving || !newAddrText.trim()" class="text-xs font-bold text-white bg-blue-600 hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed px-2.5 py-1.5 rounded-md shrink-0" x-text="addrSaving ? @json(__('pos.saving_dots')) : @json(__('pos.save_btn'))"></button>
                                </div>
                            </div>
                        </td>
                    </tr>
    ...
    <div class="mt-4 text-xs text-gray-400 text-center">
        {{ __('pos.fbr_customers_exclusive_note') }}
    </div>
    ...
    function custRow(customerId) {
        return {
            ...
            async loadAddresses() {
                ...
                try {
                    const res = await fetch('/fbr-pos/api/customer-addresses?customer_id=' + customerId, { headers: { 'Accept': 'application/json' } });
            ...
            // Server enforces the duplicate + 15-address guards; surface its message on 422.
            async saveAddress() {
    ...
    // Client-side filter over the rendered rows — matches
    // name/phone/email/city/CNIC/NTN via data-search (within the current page).
    (function () {
