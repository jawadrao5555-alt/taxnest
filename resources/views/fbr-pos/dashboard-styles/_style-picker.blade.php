<div class="relative" x-data="{ styleOpen: false, currentStyle: '{{ $dashboardStyle ?? 'default' }}' }">
    <button @click.stop="styleOpen = !styleOpen" class="p-2.5 rounded-xl bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 text-gray-900 dark:text-white hover:text-blue-600 dark:hover:text-blue-400 transition shadow-md" title="{{ __('pos.dashboard_style') }}">
    ...
    <div x-show="styleOpen" x-transition:enter="transition ease-out duration-150" x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100" x-transition:leave="transition ease-in duration-100" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0 scale-95" class="absolute right-0 mt-2 w-80 bg-white dark:bg-gray-900 rounded-2xl shadow-2xl border border-gray-300 dark:border-gray-600 z-50 p-4" @click.away="styleOpen = false" x-cloak>
        <p class="text-sm font-black text-gray-900 dark:text-white uppercase tracking-wider mb-3 px-1">{{ __('pos.dashboard_design') }}</p>
        <div class="space-y-1.5">
            @php
            $styles = [
                ['id' => 'default', 'name' => __('pos.style_square_classic'), 'desc' => __('pos.style_square_classic_desc'), 'icon' => '◻', 'colors' => ['#f3f4f6','#e5e7eb','#d1d5db']],
                ['id' => 'toast', 'name' => __('pos.style_toast_analytics'), 'desc' => __('pos.style_toast_analytics_desc'), 'icon' => '📊', 'colors' => ['#fbbf24','#f59e0b','#d97706']],
                ['id' => 'lightspeed', 'name' => __('pos.style_lightspeed_grid'), 'desc' => __('pos.style_lightspeed_grid_desc'), 'icon' => '⚡', 'colors' => ['#8b5cf6','#6366f1','#4f46e5']],
                ['id' => 'clover', 'name' => __('pos.style_clover_insights'), 'desc' => __('pos.style_clover_insights_desc'), 'icon' => '🍀', 'colors' => ['#22c55e','#16a34a','#15803d']],
                ['id' => 'oscar', 'name' => __('pos.style_oscar_pakistan'), 'desc' => __('pos.style_oscar_pakistan_desc'), 'icon' => '🇵🇰', 'colors' => ['#0ea5e9','#0284c7','#0369a1']],
                ['id' => 'shopify', 'name' => __('pos.style_shopify_modern'), 'desc' => __('pos.style_shopify_modern_desc'), 'icon' => '✨', 'colors' => ['#1e293b','#334155','#475569']],
            ];
            @endphp
            @foreach($styles as $s)
