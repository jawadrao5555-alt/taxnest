...
<div class="space-y-4">
    <div class="flex items-center justify-between fbc-a fbc-1">
        <div class="flex items-center gap-2">
            ...
            <div><h1 class="text-lg font-black text-gray-900 dark:text-white">{{ $company->name ?? __('pos.business_fallback') }}</h1><p class="text-[10px] text-gray-400">{{ now()->format('l, d M Y') }} — FBR POS</p></div>
        </div>
        <div class="flex items-center gap-2">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-4 gap-4">
        <div class="fbc-sidebar p-5 lg:row-span-2 fbc-a fbc-2">
            ...
            <div class="grid grid-cols-2 gap-2 mb-3">
                <div class="fbc-metric"><p class="text-[9px] text-emerald-200/60 font-bold">{{ __('pos.orders_caps') }}</p><p class="text-lg font-black text-white">{{ $todayStats->count ?? 0 }}</p></div>
                <div class="fbc-metric"><p class="text-[9px] text-emerald-200/60 font-bold">{{ __('pos.tax_caps') }}</p><p class="text-lg font-black text-white">Rs.{{ number_format($todayStats->tax ?? 0) }}</p></div>
            </div>

            <div class="grid grid-cols-2 gap-2 mb-4">
                <div class="fbc-metric"><p class="text-[9px] text-emerald-200/60 font-bold">{{ __('pos.fbr_done_caps') }}</p><p class="text-lg font-black text-emerald-300">{{ $fbrSubmitted }}</p></div>
                <div class="fbc-metric"><p class="text-[9px] text-emerald-200/60 font-bold">{{ __('pos.pending_caps') }}</p><p class="text-lg font-black text-amber-300">{{ $fbrPending }}</p></div>
            </div>

            <div class="border-t border-white/10 pt-3 mb-4 space-y-2">
                <div class="flex items-center justify-between"><span class="text-[9px] text-emerald-200/60 font-bold">{{ __('pos.monthly_caps') }}</span><span class="text-sm font-black text-white">Rs.{{ number_format($monthStats->revenue ?? 0) }}</span></div>
                <div class="flex items-center justify-between"><span class="text-[9px] text-emerald-200/60 font-bold">{{ __('pos.m_tax_caps') }}</span><span class="text-sm font-black text-cyan-300">Rs.{{ number_format($monthStats->tax ?? 0) }}</span></div>
            </div>

            <div x-data="{ fbrEnabled: {{ $fbrReportingStatus ? 'true' : 'false' }}, loading: false }" class="fbc-metric mb-3">
                <div class="flex items-center justify-between">
                    <span class="text-[9px] text-emerald-200/60 font-bold">{{ __('pos.fbr_reporting_caps') }}</span>
        ...
        <div class="lg:col-span-3 space-y-4">
            <div class="fbc-card overflow-hidden fbc-a fbc-3">
                <div class="px-4 py-3 border-b border-gray-100 dark:border-gray-800 flex items-center justify-between">
                    <h2 class="text-[11px] font-bold text-gray-900 dark:text-white uppercase">{{ __('pos.recent_transactions') }}</h2>
                    <a href="{{ route('fbrpos.transactions') }}" class="text-[10px] font-bold text-emerald-600 hover:text-emerald-700">{{ __('pos.view_all_caps_arrow') }}</a>
                </div>
                <div class="overflow-x-auto"><table class="w-full"><thead><tr class="bg-gray-50/60 dark:bg-gray-800/30"><th class="text-left text-[9px] text-gray-400 font-bold uppercase py-2 px-4">{{ __('pos.invoice_word') }}</th><th class="text-left text-[9px] text-gray-400 font-bold uppercase py-2 px-3">{{ __('pos.customer_word') }}</th><th class="text-right text-[9px] text-gray-400 font-bold uppercase py-2 px-3">{{ __('pos.amount_word') }}</th><th class="text-center text-[9px] text-gray-400 font-bold uppercase py-2 px-3">FBR</th><th class="text-left text-[9px] text-gray-400 font-bold uppercase py-2 px-4">{{ __('pos.date_word') }}</th></tr></thead><tbody>
                    ...
                    <tr class="border-b border-gray-50 dark:border-gray-800/50 hover:bg-emerald-50/20 dark:hover:bg-emerald-900/5 transition">
                        ...
                        <td class="py-2.5 px-3 text-center"><span class="text-[8px] font-bold px-2 py-0.5 rounded-full {{ $txn->fbr_status === null ? 'bg-slate-100 text-slate-700 dark:bg-slate-800 dark:text-slate-300' : ($txn->fbr_status === 'submitted' ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' : ($txn->fbr_status === 'failed' ? 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400' : 'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400')) }}">{{ $txn->fbr_status === null ? __('pos.bill_no_fbr_word') : ucfirst($txn->fbr_status) }}</span></td>
