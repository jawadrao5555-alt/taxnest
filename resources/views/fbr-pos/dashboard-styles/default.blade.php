...
<div class="space-y-5">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 fb-anim fb-d1">
        <div>
            <h1 class="text-xl font-extrabold text-gray-900 dark:text-white">{{ __('pos.fbr_pos_dashboard') }}</h1>
            <p class="text-[11px] text-gray-400 mt-0.5">{{ now()->format('l, d M Y') }} — {{ $company->name ?? __('pos.business_fallback') }}</p>
        </div>
    ...
    <div class="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-xl px-4 py-3 flex items-center gap-3 fb-anim fb-d1">
        ...
        <div><p class="text-[11px] font-bold text-blue-800 dark:text-blue-300">{{ __('pos.integrated_with_fbr') }}</p><p class="text-[9px] text-blue-600 dark:text-blue-400">POS #: {{ $company->fbr_pos_id }} | {{ \App\Services\FbrPosSubmissionEvidenceService::requestedEnvironmentLabel($company) }}</p></div>
    </div>
    @endif
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 fb-anim fb-d2">
        <div class="fb-stat bg-gradient-to-br from-blue-500 to-blue-700 p-4 shadow-lg shadow-blue-500/15">
            <div class="relative z-10"><span class="text-[9px] font-bold uppercase tracking-wider text-blue-100/70">{{ __('pos.todays_sales') }}</span><p class="text-xl font-extrabold text-white mt-1">Rs. {{ number_format($todayStats->revenue ?? 0) }}</p></div>
        </div>
        <div class="fb-stat bg-gradient-to-br from-indigo-500 to-indigo-700 p-4 shadow-lg shadow-indigo-500/15">
            <div class="relative z-10"><span class="text-[9px] font-bold uppercase tracking-wider text-indigo-100/70">{{ __('pos.transactions') }}</span><p class="text-xl font-extrabold text-white mt-1">{{ $todayStats->count ?? 0 }}</p></div>
        </div>
        <div class="fb-stat bg-gradient-to-br from-emerald-500 to-emerald-700 p-4 shadow-lg shadow-emerald-500/15">
            <div class="relative z-10"><span class="text-[9px] font-bold uppercase tracking-wider text-emerald-100/70">{{ __('pos.fbr_submitted') }}</span><p class="text-xl font-extrabold text-white mt-1">{{ $fbrSubmitted }}</p></div>
        </div>
        <div class="fb-stat bg-gradient-to-br from-amber-500 to-amber-700 p-4 shadow-lg shadow-amber-500/15">
            <div class="relative z-10"><span class="text-[9px] font-bold uppercase tracking-wider text-amber-100/70">{{ __('pos.fbr_pending') }}</span><p class="text-xl font-extrabold text-white mt-1">{{ $fbrPending }}</p></div>
        </div>
    </div>

    <div class="grid grid-cols-1 lg:grid-cols-3 gap-3 fb-anim fb-d3">
        <div class="fb-card p-4"><p class="text-[9px] font-bold text-gray-400 uppercase mb-1">{{ __('pos.monthly_revenue') }}</p><p class="text-lg font-extrabold text-gray-900 dark:text-white">Rs. {{ number_format($monthStats->revenue ?? 0) }}</p><p class="text-[9px] text-gray-400 mt-0.5">{{ __('pos.n_transactions', ['count' => $monthStats->count ?? 0]) }}</p></div>
        <div class="fb-card p-4"><p class="text-[9px] font-bold text-gray-400 uppercase mb-1">{{ __('pos.monthly_tax') }}</p><p class="text-lg font-extrabold text-gray-900 dark:text-white">Rs. {{ number_format($monthStats->tax ?? 0) }}</p></div>
        <div class="fb-card p-4"><p class="text-[9px] font-bold text-gray-400 uppercase mb-1">{{ __('pos.todays_tax') }}</p><p class="text-lg font-extrabold text-gray-900 dark:text-white">Rs. {{ number_format($todayStats->tax ?? 0) }}</p></div>
    </div>
    ...
    <div class="fb-card overflow-hidden fb-anim fb-d4">
        <div class="px-4 py-3 border-b border-gray-100 dark:border-gray-800 flex items-center justify-between">
            ...
            <a href="{{ route('fbrpos.transactions') }}" class="text-[10px] font-bold text-blue-600">{{ __('pos.view_all_caps') }}</a>
        </div>
        <div class="overflow-x-auto"><table class="w-full"><thead><tr class="bg-gray-50/60 dark:bg-gray-800/30"><th class="text-left text-[9px] text-gray-400 font-bold uppercase py-2 px-4">{{ __('pos.invoice_word') }}</th><th class="text-left text-[9px] text-gray-400 font-bold uppercase py-2 px-3">{{ __('pos.customer_word') }}</th><th class="text-right text-[9px] text-gray-400 font-bold uppercase py-2 px-3">{{ __('pos.amount_word') }}</th><th class="text-center text-[9px] text-gray-400 font-bold uppercase py-2 px-3">{{ __('pos.fbr_status') }}</th><th class="text-left text-[9px] text-gray-400 font-bold uppercase py-2 px-4">{{ __('pos.date_word') }}</th></tr></thead><tbody>
            ...
            <tr class="border-b border-gray-50 dark:border-gray-800/50 hover:bg-blue-50/30 dark:hover:bg-blue-900/5 transition">
                ...
                <td class="py-2 px-3 text-center"><span class="text-[8px] font-bold px-1.5 py-0.5 rounded-full {{ $txn->fbr_status === null ? 'bg-slate-100 text-slate-700 dark:bg-slate-800 dark:text-slate-300' : ($txn->fbr_status === 'submitted' ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' : ($txn->fbr_status === 'failed' ? 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400' : 'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400')) }}">{{ $txn->fbr_status === null ? __('pos.bill_no_fbr_word') : ucfirst($txn->fbr_status) }}</span></td>
                <td class="py-2 px-4 text-[10px] text-gray-400">{{ $txn->created_at->format('d M Y h:i A') }}</td>
            </tr>
            @empty
            <tr><td colspan="5" class="py-8 text-center text-[11px] text-gray-400">{{ __('pos.no_transactions_yet') }}</td></tr>
            @endforelse
        </tbody></table></div>
    </div>
</div>
