...
<div class="space-y-5">
        ...
        <div class="relative z-10">
            ...
            <div class="bg-white/10 backdrop-blur-sm rounded-2xl p-5">
                ...
                <div class="flex gap-8 mt-4">
                    <div><p class="text-2xl font-black text-white">{{ $todayStats->count ?? 0 }}</p><p class="text-[9px] text-violet-200/50 font-bold uppercase">{{ __('pos.orders_word') }}</p></div>
                    <div><p class="text-2xl font-black text-emerald-300">{{ $fbrSubmitted }}</p><p class="text-[9px] text-violet-200/50 font-bold uppercase">{{ __('pos.fbr_done') }}</p></div>
                    <div><p class="text-2xl font-black text-amber-300">{{ $fbrPending }}</p><p class="text-[9px] text-violet-200/50 font-bold uppercase">{{ __('pos.pending_word') }}</p></div>
                </div>
            </div>
            <div class="mt-4">
    ...
    <div class="bg-violet-50 dark:bg-violet-900/20 border border-violet-200 dark:border-violet-800 rounded-xl px-4 py-2.5 flex items-center gap-3 fbl-a fbl-2">
        ...
        <p class="text-[10px] font-bold text-violet-800 dark:text-violet-300">{{ __('pos.fbr_integrated_pos_line', ['id' => $company->fbr_pos_id, 'env' => \App\Services\FbrPosSubmissionEvidenceService::requestedEnvironmentLabel($company)]) }}</p>
    </div>
    @endif

    <div class="fbl-glass p-5 fbl-a fbl-3">
        <p class="text-[9px] font-bold text-gray-400 uppercase tracking-widest mb-3">{{ __('pos.monthly_overview') }}</p>
        <div class="grid grid-cols-2 sm:grid-cols-3 gap-4">
            <div><p class="text-lg font-black text-gray-900 dark:text-white">Rs.{{ number_format($monthStats->revenue ?? 0) }}</p><p class="text-[9px] text-gray-400 font-bold">{{ __('pos.revenue_n_txns', ['count' => $monthStats->count ?? 0]) }}</p></div>
            <div><p class="text-lg font-black text-cyan-600">Rs.{{ number_format($monthStats->tax ?? 0) }}</p><p class="text-[9px] text-gray-400 font-bold">{{ __('pos.monthly_tax') }}</p></div>
            <div><p class="text-lg font-black text-violet-600">Rs.{{ number_format($todayStats->tax ?? 0) }}</p><p class="text-[9px] text-gray-400 font-bold">{{ __('pos.todays_tax') }}</p></div>
        </div>
    </div>

    <div class="fbl-glass overflow-hidden fbl-a fbl-4">
        <div class="px-5 py-3 border-b border-gray-100 dark:border-gray-800 flex items-center justify-between"><h3 class="text-xs font-bold text-gray-900 dark:text-white uppercase">{{ __('pos.transactions') }}</h3><a href="{{ route('fbrpos.transactions') }}" class="text-[10px] font-bold text-violet-600">{{ __('pos.view_all_caps') }}</a></div>
        <div class="overflow-x-auto"><table class="w-full"><thead><tr class="text-left text-[9px] font-bold text-gray-400 uppercase bg-gray-50/50 dark:bg-gray-800/30"><th class="py-2.5 px-5">{{ __('pos.invoice_word') }}</th><th class="py-2.5 px-3">{{ __('pos.customer_word') }}</th><th class="py-2.5 px-3 text-right">{{ __('pos.amount_word') }}</th><th class="py-2.5 px-3 text-center">FBR</th><th class="py-2.5 px-3">{{ __('pos.date_word') }}</th></tr></thead><tbody>
            @forelse($recentTransactions as $txn)
            <tr class="border-b border-gray-50 dark:border-gray-800/50 hover:bg-violet-50/30 transition"><td class="py-2.5 px-5"><a href="{{ route('fbrpos.show', $txn->id) }}" class="text-[11px] font-bold text-violet-600">{{ $txn->invoice_number }}</a></td><td class="py-2.5 px-3 text-[11px] text-gray-600 dark:text-gray-400">{{ $txn->customer_name ?? __('pos.walk_in') }}</td><td class="py-2.5 px-3 text-right text-[11px] font-black text-gray-900 dark:text-white">Rs.{{ number_format($txn->total_amount, 2) }}</td><td class="py-2.5 px-3 text-center"><span class="text-[8px] font-bold px-1.5 py-0.5 rounded-full {{ $txn->fbr_status === null ? 'bg-slate-100 text-slate-700 dark:bg-slate-800 dark:text-slate-300' : ($txn->fbr_status === 'submitted' ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' : ($txn->fbr_status === 'failed' ? 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400' : 'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400')) }}">{{ $txn->fbr_status === null ? __('pos.bill_no_fbr_word') : ucfirst($txn->fbr_status) }}</span></td><td class="py-2.5 px-3 text-[10px] text-gray-400">{{ $txn->created_at->format('d M h:i A') }}</td></tr>
