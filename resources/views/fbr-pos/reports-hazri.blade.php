...
<div class="max-w-5xl mx-auto">
    ...
    {{-- ═══ Staff Hazri — FBR mirror (Task #560) — ADMIN/MANAGER-ONLY ═══
         ...
         Biometric punches + payroll range summary added Aug 2026 (FBR port
         of the PRA hazri upgrades — same shared row builders). --}}
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
        ...
        <div class="flex items-center gap-2 flex-wrap">
            ...
            <a href="{{ route('fbrpos.bio-sync.setup') }}" class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-gray-300 dark:border-gray-600 text-xs font-semibold text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800 transition">
    ...
    {{-- ── Biometric Punches (FBR port, Aug 2026) ─────────────────────── --}}
    <div class="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 shadow-md overflow-hidden mb-6">
        <div class="px-5 py-3.5 bg-gray-50 dark:bg-gray-800 border-b border-gray-100 dark:border-gray-700 flex items-center justify-between gap-3">
            <div class="flex items-center gap-2">
                <svg class="w-4 h-4 text-blue-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 3H5a2 2 0 00-2 2v4m6-6h10a2 2 0 012 2v4M9 3v18m0 0h10a2 2 0 002-2V9M9 21H5a2 2 0 01-2-2V9m0 0h18"/></svg>
                <span class="text-sm font-bold text-gray-800 dark:text-white">{{ __('pos.bio_hazri_section') }}</span>
                @if(!empty($unmappedPinCount) && auth('fbrpos')->user()?->isPosAdmin())
                <a href="{{ route('fbrpos.bio-sync.setup') }}" class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-amber-100 dark:bg-amber-900/40 text-amber-700 dark:text-amber-300 text-[10px] font-bold hover:bg-amber-200 dark:hover:bg-amber-800/60 transition" title="{{ __('pos.bio_unmapped_badge_title') }}">
            ...
            @if(auth('fbrpos')->user()?->isPosAdmin())
            <a href="{{ route('fbrpos.bio-sync.setup') }}" class="text-xs text-blue-600 dark:text-blue-400 hover:underline">{{ __('pos.bio_hazri_setup_link') }} →</a>
            @endif
        </div>
        ...
        <div class="p-8 text-center">
            ...
            @if(auth('fbrpos')->user()?->isPosAdmin())
            <a href="{{ route('fbrpos.bio-sync.setup') }}" class="mt-2 inline-flex items-center gap-1 text-xs text-blue-600 dark:text-blue-400 hover:underline">
            ...
            <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                ...
                <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition">
                    ...
                    <td class="px-3 py-3 text-center font-medium text-gray-700 dark:text-gray-200">
                        ...
                        @if(!empty($bp->late_minutes))
                        <span class="ml-1 inline-flex items-center px-1.5 py-0.5 rounded-full bg-red-100 dark:bg-red-900/40 text-red-700 dark:text-red-300 text-[10px] font-bold whitespace-nowrap" title="{{ __('pos.bio_late_after_title') }}">
                            {{ __('pos.bio_late_chip', ['mins' => \App\Support\PosHazriDutyHours::format($bp->late_minutes)]) }}
                        </span>
                        @endif
                    </td>
                    <td class="px-3 py-3 text-center font-medium text-gray-700 dark:text-gray-200">
                    ...
                    <td class="px-3 py-3 text-center">
                        ...
                        <span class="inline-block px-1.5 py-0.5 rounded-full text-[10px] font-bold
                            {{ $src === 'adms' ? 'bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300' : 'bg-teal-100 dark:bg-teal-900/30 text-teal-700 dark:text-teal-300' }}">
    ...
    {{-- ══════════════════════════════════════════════════════════════════════
         ── Payroll Summary Section (FBR mirror of PRA Task #280) ──────────
    ...
    <div id="payroll-summary" class="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 shadow-md overflow-hidden print:shadow-none">
        ...
        <div class="px-5 py-3.5 bg-indigo-50 dark:bg-indigo-950/30 border-b border-indigo-100 dark:border-indigo-900/50 flex items-center justify-between gap-3 flex-wrap">
            ...
            <div class="payroll-screen-only flex items-center gap-2">
                ...
                <a href="{{ route('fbrpos.reports.hazri.payroll-pdf', ['date_from' => $dateFrom, 'date_to' => $dateTo]) }}"
                   target="_blank"
        ...
        <div class="payroll-screen-only px-5 py-4 border-b border-gray-100 dark:border-gray-800 bg-gray-50/50 dark:bg-gray-800/20">
            <form method="GET" action="{{ route('fbrpos.reports.hazri') }}#payroll-summary" class="flex flex-wrap items-end gap-3">
        ...
        <table class="w-full text-sm">
            <thead class="bg-gray-50 dark:bg-gray-800 text-[11px] uppercase tracking-wide text-gray-500 dark:text-gray-400">
                <tr>
                    ...
                    <th class="px-3 py-3 text-center">{{ __('pos.bio_late_days') }}</th>
                    @endif
                    <th class="px-3 py-3 text-center">{{ __('pos.th_total_duty') }}</th>
                </tr>
            </thead>
            <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                ...
                <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition">
                    ...
                    @if(!empty($bioLateEnabled))
                    <td class="px-3 py-3 text-center font-bold {{ ($b->late_days ?? 0) > 0 ? 'text-red-600 dark:text-red-400' : 'text-gray-400' }}">
            ...
            <tfoot class="bg-gray-100 dark:bg-gray-800 text-xs font-bold">
                <tr>
                    ...
                    <td class="px-3 py-3 text-center text-gray-700 dark:text-gray-200">—</td>
                    @if(!empty($bioLateEnabled))
                    <td class="px-3 py-3 text-center {{ $bioTotalLate > 0 ? 'text-red-600 dark:text-red-400' : 'text-gray-700 dark:text-gray-200' }}">{{ $bioTotalLate ?: '—' }}</td>
                    @endif
...
{{-- ── Print styles: only the payroll section ──────────────────────────────
     Same strategy as the PRA hazri page: visibility:hidden on body * (not
     display:none) so every ancestor stays in the layout flow, then re-show
     #payroll-summary and its descendants; position:absolute moves it to the
     page origin. --}}
<style>
@media print {
    body * {
