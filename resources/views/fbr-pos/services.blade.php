...
<div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
    ...
    <div class="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 shadow-md p-5 mb-6">
        ...
        <form method="POST" action="{{ route('fbrpos.services.store') }}" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
            @csrf
            <div>
                <label class="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">{{ __('pos.name_label') }}</label>
                <input type="text" name="name" required class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-800 dark:text-white text-sm focus:ring-blue-500 focus:border-blue-500" placeholder="{{ __('pos.ph_service_name') }}">
            </div>
            <div>
                <label class="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">{{ __('pos.price_pkr') }}</label>
                <input type="number" name="price" required step="0.01" min="0" class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-800 dark:text-white text-sm focus:ring-blue-500 focus:border-blue-500" placeholder="0.00">
            </div>
            <div>
                <label class="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">{{ __('pos.tax_rate_pct_label') }}</label>
                <input type="number" name="tax_rate" step="0.01" min="0" max="100" value="0" class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-800 dark:text-white text-sm focus:ring-blue-500 focus:border-blue-500">
            </div>
            <div>
                <label class="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">{{ __('pos.description_label') }}</label>
                <input type="text" name="description" class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-800 dark:text-white text-sm focus:ring-blue-500 focus:border-blue-500" placeholder="{{ __('pos.optional') }}">
            </div>
            <div class="flex items-center gap-3 pt-5">
                ...
                <button type="submit" class="px-4 py-2 bg-blue-600 text-white text-sm rounded-lg hover:bg-blue-700 transition">{{ __('pos.add_service') }}</button>
            </div>
        </form>
    </div>
                ...
                <tr class="border-b border-gray-100 dark:border-gray-800 {{ $loop->even ? 'bg-gray-50/50 dark:bg-gray-800/20' : '' }}" x-data="{ editing: false }">
                    ...
                    <td class="px-4 py-3" x-show="!editing">
                        <div class="flex items-center gap-2">
                            ...
                            <form method="POST" action="{{ route('fbrpos.services.delete', $service->id) }}" onsubmit="return confirm({{ Js::from(__('pos.confirm_delete_service')) }})">
                                @csrf @method('DELETE')
                    ...
                    <td colspan="6" class="px-4 py-3" x-show="editing" x-cloak>
                        <form method="POST" action="{{ route('fbrpos.services.update', $service->id) }}" class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-2 items-end">
                            @csrf @method('PUT')
                            ...
                            <div class="flex items-center gap-2 flex-wrap">
                                ...
                                <button type="submit" class="text-blue-600 text-xs font-medium hover:underline">{{ __('pos.save_btn') }}</button>
