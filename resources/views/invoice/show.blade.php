<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 min-w-0" aria-labelledby="invoice-heading">
            <div class="min-w-0">
                <nav class="flex flex-wrap items-center text-xs text-gray-500 dark:text-gray-400 mb-1.5 min-w-0">
                    ...
                    <svg class="w-3.5 h-3.5 mx-1.5 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
                    <span class="text-gray-800 dark:text-gray-200 font-semibold break-words">{{ $invoice->display_invoice_number }}</span>
                </nav>
                <div class="flex flex-wrap items-center gap-3 min-w-0">
                    ...
                    <h2 id="invoice-heading" class="font-extrabold text-2xl text-gray-900 dark:text-white leading-tight tracking-tight min-w-0 break-words">Invoice {{ $invoice->display_invoice_number }}</h2>
                </div>
            </div>
            <div class="flex items-center flex-wrap gap-2 min-w-0" id="actionButtonsBlock" aria-label="Invoice actions">
                ...
                {{-- Validate FBR Payload — preview compliance check on a draft before submission --}}
                <div x-data="fbrValidatorDraft()">
                ...
                <div x-data="lockedPdfHandler()" class="inline-flex items-center gap-2">
                    ...
                    <template x-if="whtLocked">
                        <span class="inline-flex items-center px-2.5 py-1.5 bg-blue-50 border border-blue-200 text-blue-700 rounded-lg text-xs font-medium">
                            <svg class="w-3 h-3 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"/></svg>
                            WHT <span x-text="lockedWhtRate"></span>% Locked
                        </span>
                    </template>
                ...
                <div class="px-6 py-4">
                    ...
                    <ul id="deliveryHistoryList" class="divide-y divide-gray-100 dark:divide-gray-800">
                        ...
                        <li class="py-2.5 flex items-center gap-3">
                            ...
                            @if($d->status === 'read')
                            <span class="inline-flex px-2 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800">Read</span>
                            @elseif($d->status === 'delivered')
                            <span class="inline-flex px-2 py-0.5 rounded-full text-xs font-medium bg-emerald-100 text-emerald-800">Delivered</span>
                            @elseif($d->status === 'sent')
...
<div id="fbrPendingModal" style="display:none; opacity:0; transition: opacity 400ms ease;" class="fixed inset-0 z-[60] flex items-center justify-center">
    <div class="absolute inset-0 bg-black/40" style="transition: opacity 400ms ease;" onclick="closeFbrPendingModal()"></div>
    <div id="fbrPendingCard" class="relative bg-white dark:bg-gray-900 rounded-2xl shadow-2xl max-w-md w-full mx-4 overflow-hidden" style="transform: scale(0.85) translateY(30px); opacity: 0; transition: transform 400ms cubic-bezier(0.34,1.56,0.64,1), opacity 400ms ease;">
