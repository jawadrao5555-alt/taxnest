                ...
                <div class="tn-fbr-header-row flex items-center justify-between px-3 sm:px-5 h-12">
                    ...
                    <div class="tn-fbr-header-actions flex items-center gap-2 min-w-0 max-w-full">
                        ...
                        <div class="relative" x-data="{ bellOpen: false, unseen: {{ (int) $whatsNewUnseenCount }},
                            ...
                            <button data-tn-topnav-control="notification" @click="bellOpen = !bellOpen" title="{{ __('pos.ti_app_updates') }}" class="relative p-2 rounded-lg text-white hover:bg-white/15 transition cursor-pointer">
                                ...
                                <div class="max-h-96 overflow-y-auto divide-y divide-gray-100 dark:divide-gray-800">
                                    @foreach($whatsNewList as $wnu)
                                        <button type="button"
                                                ...
                                                class="block w-full px-4 py-3 text-left hover:bg-blue-50 dark:hover:bg-blue-900/20 transition cursor-pointer">
                        ...
                        <div class="relative">
                            <button data-tn-topnav-control="theme" @click="themeOpen = !themeOpen; profileOpen = false" class="p-2 rounded-lg text-white hover:bg-white/15 transition" title="{{ __('pos.ti_customize_theme') }}">
                        ...
                        <div class="relative">
                            <button data-tn-topnav-control="profile" @click="profileOpen = !profileOpen" class="flex items-center gap-2 px-2 py-1 rounded-lg hover:bg-white/10 transition group">
                            ...
                            <div x-show="profileOpen" @click.away="profileOpen = false" x-cloak
                                ...
                                <div class="py-1">
                                    ...
                                    {{-- Per-branch stock (Task 1365): only worth showing to someone
                                         who actually has two shops to move maal between. --}}
                                    @if(\App\Services\BranchStockService::canTransfer(auth('fbrpos')->user()->company_id ?? 0))
                                    <a href="{{ route('fbrpos.stock.transfers') }}" class="menu-link flex items-center gap-3 px-4 py-2 text-sm text-gray-700 dark:text-gray-300">
                                    ...
                                    @endif
                                    {{-- 💊 Pharmacy mode (Task 1558) — vanishes entirely when the
                                         mode is off, so no screen is advertised that would 403. --}}
                                    @if($fbrPharmacyLive)
                                    <a href="{{ route('fbrpos.pharmacy.batches') }}" class="menu-link flex items-center gap-3 px-4 py-2 text-sm text-gray-700 dark:text-gray-300">
                                        <svg class="w-4 h-4 text-emerald-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10"/></svg>
                                        {{ __('pos.ph_nav_batches') }}
                                    </a>
                                    <a href="{{ route('fbrpos.pharmacy.claims') }}" class="menu-link flex items-center gap-3 px-4 py-2 text-sm text-gray-700 dark:text-gray-300">
                                    ...
                                    <a href="{{ route('fbrpos.pharmacy.reports') }}" class="menu-link flex items-center gap-3 px-4 py-2 text-sm text-gray-700 dark:text-gray-300">
                                ...
                                <div class="border-t border-gray-100 dark:border-gray-700 py-1">
                                    ...
                                    @endif
                                    @if ($fbrUser?->role === 'company_admin')
                                    <a href="{{ route('fbrpos.customize') }}" class="menu-link flex items-center gap-3 px-4 py-2 text-sm text-gray-700 dark:text-gray-300 {{ request()->routeIs('fbrpos.customize') ? 'bg-blue-50 dark:bg-blue-900/20' : '' }}">
                                        <svg class="w-4 h-4 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6V4m0 2a2 2 0 100 4m0-4a2 2 0 110 4m-6 8a2 2 0 100-4m0 4v2m0-6V4m12 10a2 2 0 100 4m0-4V4"/></svg>
                                        {{ __('pos.nav_customize_pos') }}
                                        <x-new-badge page="fbrpos.customize" panel="fbrpos" class="ml-auto" />
                                    </a>
                                    @endif
                ...
                <nav class="flex-1 overflow-y-auto px-2 py-3 space-y-4 main-scroll">
                    ...
                    <div>
                        ...
                        {{-- Per-branch stock (Task 1365): only worth showing to someone
                             who actually has two shops to move maal between. --}}
                        @if(\App\Services\BranchStockService::canTransfer(auth('fbrpos')->user()->company_id ?? 0))
                        <a href="{{ route('fbrpos.stock.transfers') }}" class="{{ $sidebarBase }} {{ request()->routeIs('fbrpos.stock.transfers') ? $sidebarActive : $sidebarInactive }}">
                            <svg class="w-4 h-4 opacity-80" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4"/></svg>
                            {{ __('pos.branch_transfer') }}
                        </a>
                        @endif
                        {{-- 💊 Pharmacy mode (Task 1558) — see the header menu note. --}}
                        @if($fbrPharmacyLive)
                        <a href="{{ route('fbrpos.pharmacy.batches') }}" class="{{ $sidebarBase }} {{ request()->routeIs('fbrpos.pharmacy.batches') ? $sidebarActive : $sidebarInactive }}">
                            <svg class="w-4 h-4 opacity-80" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10"/></svg>
                            {{ __('pos.ph_nav_batches') }}
                        </a>
                        <a href="{{ route('fbrpos.pharmacy.claims') }}" class="{{ $sidebarBase }} {{ request()->routeIs('fbrpos.pharmacy.claim*') ? $sidebarActive : $sidebarInactive }}">
