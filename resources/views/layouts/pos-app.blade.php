...
@php
    ...
    // Deals management is privileged and plan-bound. Custom-access users must
    // have the existing Customize grant explicitly; an absent grant defaults
    // to denied for cashiers and every confined role.
    $dealsNavVisible = $posUserLayout
        && $posUserLayout->isPosAdmin()
        && $posNavCan('customize', !$isCashierLayout)
        && \App\Services\PosFeatureService::planAllows($companyLayout, 'deals_enabled');
...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}" class="{{ $isDarkMode ? 'dark' : '' }}">
    ...
    <body class="tn-premium-shell pos-layout-root h-screen overflow-hidden antialiased" data-theme="{{ $posTheme }}"@if($posEffStyleLayout === 'saaf') data-saaf="1"@endif>
        <x-pwa-init />
        <div class="flex flex-col h-full" x-data="{ profileOpen: false, mobileMenuOpen: false, themeOpen: false, currentTheme: '{{ $posTheme }}', guidedOn: {{ ($companyLayout->pos_guided_flow_enabled ?? true) ? 'true' : 'false' }} }" @keydown.escape.window="profileOpen = false; mobileMenuOpen = false; themeOpen = false">
            ...
            <header class="topnav-bar flex-shrink-0 relative z-50 {{ is_array(session('impersonation')) ? 'tn-impersonated-header' : '' }}">
                    ...
                    <div class="tn-impersonation-header-left flex items-center gap-3 flex-shrink-0">
                        ...
                        <nav class="tn-impersonation-header-nav hidden lg:flex items-center gap-1">
                            ...
                            <a href="{{ route('pos.customize') }}"
                               class="nav-pill px-2.5 py-1.5 rounded-lg text-[11px] font-medium {{ request()->routeIs('pos.customize') ? 'active text-white' : 'text-white' }}">{{ __('pos.settings') }}<x-new-badge page="pos.customize" panel="pos" dot class="ml-1" /></a>
                            @endif
                            @endif
                        </nav>
                        ...
                        <div class="relative">
                            ...
                            <div x-show="profileOpen" x-cloak @click.outside="profileOpen = false"
                                ...
                                <div class="py-1.5 max-h-[65vh] overflow-y-auto">
                                    ...
                                    {{-- Navigation must describe what the shop can use now. When Inventory is
                                         OFF, hide the whole section instead of advertising dead-end links. --}}
                                    @if($inventoryEnabledLayout && $posNavCan('inventory', !$isCashierLayout))
                                    <div class="px-3 pt-3 pb-1">
                                        <p class="text-[9px] font-bold uppercase tracking-widest text-gray-400 dark:text-gray-600">{{ __('pos.nav_inventory') }}</p>
                                    </div>
                                    ...
                                    <a href="{{ route('pos.customize') }}" class="menu-link flex items-center gap-2.5 px-4 py-2.5 text-[12px] font-bold text-gray-800 dark:text-gray-100 {{ request()->routeIs('pos.customize') ? 'bg-purple-50 dark:bg-purple-900/20' : '' }}">
                                        ...
                                        <x-new-badge page="pos.customize" panel="pos" />
                ...
                <div data-mobile-nav x-show="mobileMenuOpen" x-cloak
                    ...
                    @endif
                    @if($posNavCan('customize', !$isCashierLayout))
                    <a href="{{ route('pos.customize') }}" class="nav-pill px-3 py-1.5 rounded-lg text-[11px] font-medium text-white">{{ __('pos.settings') }}<x-new-badge page="pos.customize" panel="pos" dot class="ml-1" /></a>
                    @endif
                    @endif
