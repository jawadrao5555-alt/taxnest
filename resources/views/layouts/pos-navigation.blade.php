<div class="tn-nav-brand p-4 border-b border-gray-200 dark:border-gray-800">
    <a href="/pos/dashboard" class="flex items-center space-x-2">
        <div class="tn-nav-mark w-8 h-8 rounded-lg bg-gradient-to-br from-purple-500 to-violet-600 flex items-center justify-center">
...
<div class="px-2 py-4 space-y-1">
    ...
    @php
        ...
        $inventoryEnabled = \App\Services\PosFeatureService::moduleAvailable($companyNav, 'inventory');
        $posUserNav = auth('pos')->user();
        $isCashierNav = $posUserNav && $posUserNav->isPosCashier();
        $recipesNavInv = \App\Services\PosFeatureService::moduleAvailable($companyNav, 'recipes');
    @endphp
    ...
    <a href="{{ route('pos.inventory-master') }}" class="sidebar-link flex items-center gap-3 py-3 px-4 rounded-lg text-sm {{ request()->routeIs('pos.inventory-master') || request()->routeIs('pos.inventory-master.*') ? 'active text-gray-900 dark:text-white' : 'text-gray-600 dark:text-gray-400' }}">
        <svg class="w-5 h-5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"/></svg>
    ...
    <a href="{{ route('pos.inventory.stock-check.index') }}" class="sidebar-link flex items-center gap-3 py-3 px-4 rounded-lg text-sm {{ request()->routeIs('pos.inventory.stock-check.*') ? 'active text-gray-900 dark:text-white' : 'text-gray-600 dark:text-gray-400' }}">
        <svg class="w-5 h-5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4"/></svg>
    ...
    @php
        ...
        $companyNav = $companyNav ?? \App\Models\Company::find(app('currentCompanyId'));
    ...
    @if($featNav->kitchen)
    <a href="{{ route('pos.restaurant.kitchen-settings') }}" class="sidebar-link flex items-center gap-3 py-3 px-4 rounded-lg text-sm {{ request()->routeIs('pos.restaurant.kitchen-settings') ? 'active text-gray-900 dark:text-white' : 'text-gray-600 dark:text-gray-400' }}">
        <svg class="w-5 h-5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.066 2.573c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.573 1.066c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.066-2.573c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/></svg>
        Kitchen Settings
    </a>
    @endif
    ...
    @php $serviceWorkflowNav = \App\Services\PosServiceWorkflowProfiles::forCompany($companyNav); @endphp
    <a href="{{ route('pos.service-work-orders.index') }}" class="sidebar-link flex items-center gap-3 py-3 px-4 rounded-lg text-sm {{ request()->routeIs('pos.service-work-orders.*') ? 'active text-gray-900 dark:text-white' : 'text-gray-600 dark:text-gray-400' }}">
        <svg class="w-5 h-5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M9 12h6m-6 4h6M7 4h10a2 2 0 012 2v14H5V6a2 2 0 012-2z"/></svg>
        {{ $serviceWorkflowNav['noun'] }} Board
    </a>
    @endif

    @if(!$isCashier && \App\Services\PosFeatureService::moduleRelevant($companyNav, 'service_jobs'))
