...
<div class="mt-4" x-data="{ activeTab: 'dashboard' }">
    <div class="flex gap-1 mb-4">
        <button @click="activeTab = 'dashboard'" :class="activeTab === 'dashboard' ? 'bg-purple-600 text-white shadow-sm' : 'text-gray-500 hover:bg-gray-100 dark:hover:bg-gray-800'" class="px-4 py-1.5 rounded-lg text-[10px] font-bold uppercase tracking-wider transition-all">{{ __("pos.dashboard") }}</button>
        <button @click="activeTab = 'drafts'" :class="activeTab === 'drafts' ? 'bg-purple-600 text-white shadow-sm' : 'text-gray-500 hover:bg-gray-100 dark:hover:bg-gray-800'" class="px-4 py-1.5 rounded-lg text-[10px] font-bold uppercase tracking-wider transition-all flex items-center gap-1.5">{{ __("pos.drafts_word") }} <span class="text-[8px] px-1.5 py-0.5 rounded-full" :class="activeTab === 'drafts' ? 'bg-white/20' : 'bg-amber-100 text-amber-700 dark:bg-amber-900/20 dark:text-amber-400'" x-data x-text="document.querySelectorAll('[data-draft-count]').length > 0 ? '{{ count($drafts) }}' : '{{ count($drafts) }}'">{{ count($drafts) }}</span></button>
    </div>
    ...
    <div x-show="activeTab === 'drafts'" x-cloak x-data="draftsManager()" x-init="init()">
        <div class="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 overflow-hidden">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between p-5 border-b border-gray-100/50 dark:border-gray-800/50">
                <div>
                    <h3 class="text-xs font-bold text-gray-900 dark:text-white uppercase tracking-wide">{{ __("pos.saved_drafts") }}</h3>
                    <p class="text-[10px] text-gray-400 mt-0.5">{{ __("pos.autosaved_not_finalized") }}</p>
                </div>
                        ...
                        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                            <div class="flex-1 min-w-0">
                                <div class="flex items-center gap-2 mb-1">
                                    <span class="inline-flex items-center px-1.5 py-0.5 rounded text-[8px] font-extrabold bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400 uppercase">{{ __("pos.draft_word") }}</span>
                                ...
                                <div class="flex flex-wrap items-center gap-x-3 gap-y-1 text-[9px] text-gray-400">
                                    <span x-text="draft.customer_name || @js(__('pos.walk_in'))"></span>
                                    <span x-text="(draft.items ? draft.items.length : 0) + @js(__('pos.sfx_items'))"></span>
                        ...
                        <template x-if="draft.items && draft.items.length > 0">
                            <div class="mt-2 flex flex-wrap gap-1">
                                ...
                                <template x-if="draft.items.length > 5">
                                    <span class="inline-flex items-center px-2 py-0.5 rounded-md text-[9px] bg-gray-50 text-gray-400 dark:bg-gray-800" x-text="'+' + (draft.items.length - 5) + @js(__('pos.more_suffix'))"></span>
                                </template>
                            </div>
                        </template>
                    </div>
            ...
            <template x-if="drafts.length === 0">
                <div class="p-10 text-center">
                    ...
                    <p class="text-[11px] text-gray-400">{{ __("pos.no_drafts_saved") }}</p>
                </div>
            </template>
        </div>
    </div>
