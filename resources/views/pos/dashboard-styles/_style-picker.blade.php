...
<div class="flex items-center gap-3 flex-shrink-0">
    ...
    <div class="flex items-center gap-2 px-4 py-2 rounded-xl bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 shadow-md" title="{{ __('pos.ti_standalone_pos') }}">
        <span class="w-2 h-2 rounded-full" style="background:#0d9488;"></span>
        <span class="text-xs font-extrabold text-gray-900 dark:text-white uppercase tracking-wide">{{ __("pos.standalone_mode") }}</span>
    </div>
    ...
    <div class="flex items-center gap-2.5 px-4 py-2 rounded-xl bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 shadow-md" title="{{ __('pos.ti_pra_admin_set') }}">
        <span class="text-xs font-extrabold text-gray-900 dark:text-white uppercase tracking-wide">{{ __("pos.pra_reporting") }}</span>
        @if($praAssignedOn)
        <span class="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-emerald-50 dark:bg-emerald-900/30 border border-emerald-200 dark:border-emerald-800 text-emerald-700 dark:text-emerald-300 text-xs font-black uppercase tracking-wide">
            <span class="w-2 h-2 rounded-full bg-emerald-500"></span> {{ __("pos.online") }}
        </span>
        @else
        <span class="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-gray-100 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 text-gray-500 dark:text-gray-400 text-xs font-black uppercase tracking-wide">
            <span class="w-2 h-2 rounded-full bg-gray-400"></span> {{ __("pos.offline") }}
        </span>
        @endif
    </div>
    @else
    <div x-data="{ praEnabled: {{ ($praStatus ?? auth('pos')->user()?->praReportingEnabled($company) ?? false) ? 'true' : 'false' }}, praLoading: false }" class="flex items-center gap-2.5 px-4 py-2 rounded-xl bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 shadow-md">
        ...
        <span x-text="praEnabled ? @js(__('pos.on_word')) : @js(__('pos.off_word'))" :class="praEnabled ? 'text-purple-700 font-black' : 'text-gray-600 font-extrabold'" class="text-xs"></span>
    </div>
    @endif
    @unless(auth('pos')->user()?->isPosCashier())
    <a href="{{ route('pos.customize') }}#style" class="p-2.5 rounded-xl bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 text-gray-900 dark:text-white hover:text-purple-600 dark:hover:text-purple-400 transition shadow-md" title="{{ __('pos.ti_style_themes') }}">
