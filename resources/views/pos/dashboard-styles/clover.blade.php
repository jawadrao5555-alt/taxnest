...
<div class="space-y-4 w-full">
    <div class="flex items-center justify-between cl-anim cl-d1">
        <div class="flex items-center gap-2">
            ...
            <div>
                <h1 class="text-lg font-black text-gray-900 dark:text-white">{{ $company->name ?? __("pos.business_fallback") }}</h1>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-4 gap-4">
        <div class="cl-sidebar p-5 lg:row-span-2 cl-anim cl-d2">
            <p class="text-[9px] font-bold text-emerald-300/60 uppercase tracking-widest mb-4">{{ __("pos.todays_summary") }}</p>
            ...
            <div class="cl-metric mb-4">
                <div class="flex items-center justify-between"><p class="text-[9px] text-emerald-200/60 font-bold">{{ __("pos.tables_caps") }}</p><p class="text-sm font-black text-white">{{ $occupiedTables ?? 0 }}/{{ $totalTables ?? 0 }}</p></div>
            ...
            <div class="border-t border-white/10 pt-3 mb-4 space-y-2">
                <div class="flex items-center justify-between"><span class="text-[9px] text-emerald-200/60 font-bold">{{ __("pos.profit_caps") }}</span><span class="text-sm font-black {{ ($todayProfit ?? 0) >= 0 ? 'text-emerald-300' : 'text-red-300' }}">Rs.{{ number_format($todayProfit ?? 0) }}</span></div>
                <div class="flex items-center justify-between"><span class="text-[9px] text-emerald-200/60 font-bold">{{ __("pos.cost_caps") }}</span><span class="text-sm font-black text-white">Rs.{{ number_format($todayCost ?? 0) }}</span></div>
                <div class="flex items-center justify-between"><span class="text-[9px] text-emerald-200/60 font-bold">{{ __("pos.margin_caps") }}</span><span class="text-sm font-black text-emerald-300">{{ ($todaySales ?? 0) > 0 ? round(($todayProfit ?? 0) / ($todaySales ?? 1) * 100) : 0 }}%</span></div>
                <div class="flex items-center justify-between"><span class="text-[9px] text-emerald-200/60 font-bold">{{ __("pos.tax_caps") }}</span><span class="text-sm font-black text-cyan-300">Rs.{{ number_format($todayTax ?? $todayStats->tax ?? 0) }}</span></div>
            </div>
            @endif

            <div class="border-t border-white/10 pt-3">
                <p class="text-[8px] text-emerald-200/40 font-bold uppercase tracking-widest mb-2">{{ __("pos.quick_actions") }}</p>
                @if($isRestaurant)
                <a href="{{ route('pos.invoice.create') }}" class="cl-nav-btn"><svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 7h6m0 10v-3m-3 3h.01M9 17h.01M9 14h.01M12 14h.01M15 11h.01M12 11h.01M9 11h.01M7 21h10a2 2 0 002-2V5a2 2 0 00-2-2H7a2 2 0 00-2 2v14a2 2 0 002 2z"/></svg>{{ __("pos.pos_screen") }}</a>
                ...
                <a href="{{ route('pos.restaurant.tables') }}" class="cl-nav-btn"><svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16"/></svg>{{ __("pos.tables_word") }}</a>
                <a href="{{ route('pos.restaurant.kds') }}" class="cl-nav-btn"><svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9.75 17L9 20l-1 1h8l-1-1-.75-3M3 13h18M5 17h14a2 2 0 002-2V5a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/></svg>{{ __("pos.kitchen_word") }}</a>
                <a href="{{ route('pos.products') }}" class="cl-nav-btn"><svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4"/></svg>{{ __("pos.menu_word") }}</a>
                ...
                @else
                <a href="{{ route('pos.invoice.create') }}" class="cl-nav-btn"><svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/></svg>{{ __("pos.new_sale") }}</a>
                <a href="{{ route('pos.transactions') }}" class="cl-nav-btn">{{ __("pos.orders_word") }}</a>
                <a href="{{ route('pos.products') }}" class="cl-nav-btn">{{ __("pos.products_word") }}</a>
                <a href="{{ route('pos.customers') }}" class="cl-nav-btn">{{ __("pos.customers_word") }}</a>
                <a href="{{ route('pos.reports') }}" class="cl-nav-btn">{{ __("pos.reports") }}</a>
                @endif
            </div>
        </div>
        ...
        <div class="lg:col-span-3 space-y-4">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 cl-anim cl-d3">
                <div class="sm:col-span-2 cl-card p-4">
                    <h2 class="text-[11px] font-bold text-gray-900 dark:text-white uppercase tracking-wide mb-3">{{ __("pos.weekly_revenue") }}</h2>
                    <div style="height: 180px;"><canvas id="salesChart"></canvas></div>
                </div>
                @if(isset($orderTypeCounts))
                <div class="cl-card p-4">
                    <h2 class="text-[11px] font-bold text-gray-900 dark:text-white uppercase tracking-wide mb-3">{{ __("pos.order_types") }}</h2>
            ...
            <div class="cl-card p-4 cl-anim cl-d3">
                <h3 class="text-[11px] font-bold text-gray-900 dark:text-white uppercase mb-3">{{ __("pos.payment_methods") }}</h3>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 cl-anim cl-d4">
                <div class="cl-card overflow-hidden">
                    <div class="px-4 py-3 border-b border-gray-100 dark:border-gray-800 bg-emerald-50/50 dark:bg-emerald-900/10 flex items-center justify-between">
                        <h2 class="text-[11px] font-bold text-emerald-700 dark:text-emerald-400 uppercase">🏆 {{ __("pos.top_sellers") }}</h2>
                        <a href="{{ $topItemsReportUrl ?? route('pos.reports', ['top_items' => 1]) }}" class="text-[10px] font-bold text-emerald-600 hover:text-emerald-700 dark:text-emerald-400">{{ __("pos.view_all_caps") }}</a>
                    </div>
                ...
                <div class="cl-card overflow-hidden">
                    <div class="px-4 py-3 border-b border-gray-100 dark:border-gray-800 bg-{{ ($lowStockItems ?? collect())->count() > 0 ? 'red' : 'green' }}-50/50 dark:bg-{{ ($lowStockItems ?? collect())->count() > 0 ? 'red' : 'green' }}-900/10 flex items-center justify-between">
                        <h2 class="text-[11px] font-bold {{ ($lowStockItems ?? collect())->count() > 0 ? 'text-red-700 dark:text-red-400' : 'text-green-700 dark:text-green-400' }} uppercase">📦 {{ __("pos.stock_alerts") }}</h2>
                    ...
                    <div class="p-3 space-y-1">
                        ...
                        @empty
                        <div class="text-center py-6"><svg class="w-8 h-8 mx-auto text-green-400 mb-2" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg><p class="text-[11px] text-green-600 font-bold">{{ __("pos.all_stocked") }}</p></div>
                        @endforelse
                    </div>
                </div>
            </div>
            ...
            <div class="cl-card overflow-hidden cl-anim cl-d5">
                <div class="px-4 py-3 border-b border-gray-100 dark:border-gray-800 flex items-center justify-between">
                    <h2 class="text-[11px] font-bold text-gray-900 dark:text-white uppercase">{{ $isRestaurant ? __("pos.recent_orders") : __("pos.recent_transactions") }}</h2>
                    <a href="{{ route('pos.transactions') }}" class="text-[10px] font-bold text-emerald-600 hover:text-emerald-700">{{ __("pos.view_all_arrow") }}</a>
                </div>
                <table class="w-full"><thead><tr class="bg-gray-50/60 dark:bg-gray-800/30"><th class="text-left text-[9px] text-gray-400 font-bold uppercase py-2 px-4">{{ $isRestaurant ? __("pos.order_word") : __("pos.invoice_word") }}</th><th class="text-left text-[9px] text-gray-400 font-bold uppercase py-2 px-3 hidden sm:table-cell">{{ $isRestaurant ? __("pos.type_word") : __("pos.customer_word") }}</th><th class="text-right text-[9px] text-gray-400 font-bold uppercase py-2 px-3">{{ __("pos.amount_word") }}</th><th class="text-center text-[9px] text-gray-400 font-bold uppercase py-2 px-3">{{ __("pos.status_word") }}</th></tr></thead><tbody>
                    ...
                    <tr class="border-b border-gray-50 dark:border-gray-800/50 hover:bg-emerald-50/20 dark:hover:bg-emerald-900/5 transition">
                        <td class="py-2.5 px-4 text-[11px] font-bold text-emerald-600">@if($isRestaurant) @include('pos.dashboard-styles._restaurant-order-identity', ['order' => $ro]) @else {{ $ro->invoice_number ?? ('#' . $ro->id) }} @endif</td>
                        <td class="py-2.5 px-3 hidden sm:table-cell text-[11px] text-gray-600 dark:text-gray-400">{{ $isRestaurant ? ucwords(str_replace('_', ' ', $ro->order_type ?? '-')) : ($ro->customer_name ?? __("pos.walk_in")) }}</td>
                    ...
                    @empty<tr><td colspan="4" class="py-8 text-center text-[11px] text-gray-400">{{ __("pos.no_data_yet") }}</td></tr>@endforelse
                </tbody></table>
            </div>
        </div>
    </div>
