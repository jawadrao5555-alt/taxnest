...
.os-banner { background: linear-gradient(135deg, #083D49 0%, #0A4D5C 55%, #10606F 100%); border-radius: 16px; position: relative; overflow: hidden; }
...
.os-accent-blue::before { background: #0A4D5C; }
...
<div class="space-y-4 w-full">
    ...
    <div class="os-stat-strip os-anim os-d2">
        <div class="os-stat-cell">
            ...
            <p class="text-xl font-black text-gray-900 dark:text-white" style="font-variant-numeric:tabular-nums">Rs.{{ number_format($todayTotalSale ?? $todaySales ?? $todayStats->revenue ?? 0) }}</p>
        ...
        <div class="os-stat-cell">
            <p class="text-[9px] font-bold text-teal-600 uppercase">{{ __("pos.new_customers") }}</p>
            <p class="text-xl font-black text-gray-900 dark:text-white">{{ number_format($newCustomersToday ?? 0) }}</p>
            <p class="text-[9px] text-gray-400 mt-0.5">{{ number_format($newCustomersMonth ?? 0) }} {{ __("pos.period_this_month") }}</p>
        </div>
        <div class="os-stat-cell">
            @if($isRestaurant)
            ...
            <p class="text-xl font-black text-gray-900 dark:text-white">Rs.{{ number_format($monthTotalSale ?? $monthSales ?? $monthStats->revenue ?? 0) }}</p>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-5 gap-4 os-anim os-d5">
        <div class="lg:col-span-2 os-card p-4">
            ...
            <div class="space-y-2">
                ...
                <div class="flex items-center gap-3">
                    <div class="w-8 h-8 rounded-lg flex items-center justify-center text-[10px] font-black flex-shrink-0 {{ $idx === 0 ? 'bg-teal-600 text-white' : ($idx < 3 ? 'bg-teal-100 text-teal-700 dark:bg-teal-900/30 dark:text-teal-400' : 'bg-gray-100 text-gray-400 dark:bg-gray-800') }}">{{ $idx + 1 }}</div>
                    <div class="flex-1 min-w-0">
                        <p class="text-[11px] font-bold text-gray-900 dark:text-white truncate">{{ $p->item_name }}</p>
                        <div class="w-full bg-gray-100 dark:bg-gray-800 rounded-full h-1.5 mt-1"><div class="bg-teal-500 h-1.5 rounded-full" style="width: {{ $idx === 0 ? 100 : max(20, 100 - $idx * 20) }}%"></div></div>
                    </div>
