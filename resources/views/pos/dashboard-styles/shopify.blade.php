...
<div class="space-y-5 w-full">
        ...
        <div class="relative z-10">
            <div class="flex items-start justify-between gap-4 mb-10">
                <div>
                    ...
                    <p class="text-sm text-slate-400 mt-2">{{ $company->name ?? __("pos.business_fallback") }}</p>
                </div>
    ...
    <div class="flex items-center gap-3 overflow-x-auto pb-1 sh-anim sh-d2">
        @if($isRestaurant)
        <a href="{{ route('pos.invoice.create') }}" class="flex-shrink-0 px-5 py-2.5 rounded-xl bg-indigo-600 text-white text-[11px] font-bold hover:bg-indigo-700 transition shadow-sm">{{ __("pos.pos_screen") }}</a>
        <a href="{{ route('pos.transactions') }}" class="flex-shrink-0 px-5 py-2.5 rounded-xl text-[11px] font-bold text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white transition">{{ __("pos.orders_word") }}</a>
        <a href="{{ route('pos.restaurant.tables') }}" class="flex-shrink-0 px-5 py-2.5 rounded-xl text-[11px] font-bold text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white transition">{{ __("pos.tables_word") }}</a>
        <a href="{{ route('pos.restaurant.kds') }}" class="flex-shrink-0 px-5 py-2.5 rounded-xl text-[11px] font-bold text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white transition">{{ __("pos.kitchen_word") }}</a>
        <a href="{{ route('pos.products') }}" class="flex-shrink-0 px-5 py-2.5 rounded-xl text-[11px] font-bold text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white transition">{{ __("pos.menu_word") }}</a>
        <a href="{{ route('pos.restaurant.ingredients') }}" class="flex-shrink-0 px-5 py-2.5 rounded-xl text-[11px] font-bold text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white transition">{{ __("pos.ingredients_word") }}</a>
        @else
        <a href="{{ route('pos.invoice.create') }}" class="flex-shrink-0 px-5 py-2.5 rounded-xl bg-indigo-600 text-white text-[11px] font-bold hover:bg-indigo-700 transition shadow-sm">{{ __("pos.new_sale") }}</a>
        <a href="{{ route('pos.transactions') }}" class="flex-shrink-0 px-5 py-2.5 rounded-xl text-[11px] font-bold text-gray-600 dark:text-gray-400 hover:text-gray-900 transition">{{ __("pos.orders_word") }}</a>
        <a href="{{ route('pos.products') }}" class="flex-shrink-0 px-5 py-2.5 rounded-xl text-[11px] font-bold text-gray-600 dark:text-gray-400 hover:text-gray-900 transition">{{ __("pos.products_word") }}</a>
        <a href="{{ route('pos.customers') }}" class="flex-shrink-0 px-5 py-2.5 rounded-xl text-[11px] font-bold text-gray-600 dark:text-gray-400 hover:text-gray-900 transition">{{ __("pos.customers_word") }}</a>
        <a href="{{ route('pos.reports') }}" class="flex-shrink-0 px-5 py-2.5 rounded-xl text-[11px] font-bold text-gray-600 dark:text-gray-400 hover:text-gray-900 transition">{{ __("pos.reports") }}</a>
        @endif
    </div>
    ...
    <div class="sh-minimal p-5 sh-anim sh-d2">
        <div class="flex items-center gap-8 flex-wrap">
            <div><p class="text-[9px] text-gray-400 font-bold uppercase tracking-wider">{{ __("pos.profit_word") }}</p><p class="text-xl font-black {{ ($todayProfit ?? 0) >= 0 ? 'sh-green' : 'text-red-600' }} mt-0.5">Rs.{{ number_format($todayProfit ?? 0) }}</p></div>
            <div class="w-px h-10 bg-gray-100 dark:bg-gray-800"></div>
            <div><p class="text-[9px] text-gray-400 font-bold uppercase tracking-wider">{{ __("pos.cost_word") }}</p><p class="text-xl font-black text-gray-900 dark:text-white mt-0.5">Rs.{{ number_format($todayCost ?? 0) }}</p></div>
            <div class="w-px h-10 bg-gray-100 dark:bg-gray-800"></div>
            <div><p class="text-[9px] text-gray-400 font-bold uppercase tracking-wider">{{ __("pos.margin_word") }}</p><p class="text-xl font-black text-indigo-600 mt-0.5">{{ ($todaySales ?? 0) > 0 ? round(($todayProfit ?? 0) / ($todaySales ?? 1) * 100) : 0 }}%</p></div>
            <div class="w-px h-10 bg-gray-100 dark:bg-gray-800"></div>
            <div><p class="text-[9px] text-gray-400 font-bold uppercase tracking-wider">{{ __("pos.tax_word") }}</p><p class="text-xl font-black text-gray-900 dark:text-white mt-0.5">Rs.{{ number_format($todayTax ?? $todayStats->tax ?? 0) }}</p></div>
            <div class="w-px h-10 bg-gray-100 dark:bg-gray-800"></div>
            <div><p class="text-[9px] text-gray-400 font-bold uppercase tracking-wider">{{ __("pos.discounts_word") }}</p><p class="text-xl font-black text-orange-500 mt-0.5">Rs.{{ number_format($todayDiscount ?? 0) }}</p></div>
        </div>
    </div>
    @endif
    ...
    <div class="sh-minimal p-6 sh-anim sh-d3">
        <div class="flex items-center justify-between mb-4"><h2 class="text-xs font-bold text-gray-900 dark:text-white uppercase tracking-wider">{{ __("pos.revenue_last_7_days") }}</h2></div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 sh-anim sh-d4">
        ...
        <div class="sh-minimal p-5">
            <h2 class="text-xs font-bold text-gray-900 dark:text-white uppercase tracking-wider mb-4">{{ __("pos.order_types") }}</h2>
        ...
        <div class="sh-minimal overflow-hidden">
            ...
            <div class="divide-y divide-gray-50 dark:divide-gray-800/50">
                ...
                <div class="flex items-center gap-4 px-5 py-3 hover:bg-gray-50/50 dark:hover:bg-gray-800/30 transition">
                    ...
                    <div class="flex-1 min-w-0"><p class="text-[12px] font-bold text-gray-900 dark:text-white truncate">{{ $p->item_name }}</p><p class="text-[9px] text-gray-400 mt-0.5">{{ $p->total_qty }} {{ __("pos.sold_suffix") }}</p></div>
                    <p class="text-[12px] font-black text-gray-900 dark:text-white">Rs.{{ number_format($p->total_revenue) }}</p>
                </div>
                @empty<div class="py-8 text-center text-[11px] text-gray-400">{{ __("pos.no_data") }}</div>@endforelse
            </div>
        </div>
        @endif
        ...
        <div class="sh-minimal p-5">
            <h3 class="text-xs font-bold text-gray-900 dark:text-white uppercase tracking-wider mb-4">{{ __("pos.payment_split") }}</h3>
            ...
            <div class="flex items-center gap-3 py-3 border-b border-gray-50 dark:border-gray-800/50 last:border-0">
                ...
                <div class="flex-1"><p class="text-[12px] font-bold text-gray-900 dark:text-white">{{ \App\Support\PosPaymentLabels::label($pb->payment_method) }}</p><p class="text-[9px] text-gray-400">{{ $pb->count }} {{ __("pos.transactions_lc") }}</p></div>
                <p class="text-sm font-black text-gray-900 dark:text-white">Rs.{{ number_format($pb->total) }}</p>
            </div>
            @empty<p class="text-[11px] text-gray-400 py-6 text-center">{{ __("pos.no_sales_today") }}</p>@endforelse
        </div>
        @endif
    </div>
    ...
    <div class="sh-minimal overflow-hidden sh-anim sh-d4">
        <div class="px-5 py-3 border-b border-gray-100 dark:border-gray-800/50 flex items-center justify-between bg-{{ ($lowStockItems ?? collect())->count() > 0 ? 'amber-50/30 dark:bg-transparent' : 'transparent' }}">
            <h2 class="text-xs font-bold text-gray-900 dark:text-white uppercase tracking-wider">{{ __("pos.inventory_word") }}</h2>
            @if(($lowStockItems ?? collect())->count() > 0)<span class="text-[9px] font-bold px-2 py-0.5 rounded-full bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400">{{ $lowStockItems->count() }} {{ __("pos.low_lc") }}</span>@endif
        </div>
        ...
        @else
        <div class="py-8 text-center"><p class="text-[12px] sh-green font-bold">✓ {{ __("pos.all_ingredients_in_stock") }}</p></div>
        @endif
    </div>
    @endif
    ...
    <div class="sh-minimal overflow-hidden sh-anim sh-d5">
        <div class="px-5 py-3 border-b border-gray-100 dark:border-gray-800/50 flex items-center justify-between">
            <h2 class="text-xs font-bold text-gray-900 dark:text-white uppercase tracking-wider">{{ $isRestaurant ? __("pos.recent_orders") : __("pos.recent_transactions") }}</h2>
            <a href="{{ route('pos.transactions') }}" class="sh-link">{{ __("pos.view_all_arrow_lc") }}</a>
        </div>
        <table class="w-full"><thead><tr class="text-left text-[9px] font-bold text-gray-400 uppercase tracking-wider bg-gray-50/40 dark:bg-gray-800/20"><th class="py-2.5 px-5">{{ $isRestaurant ? __("pos.order_word") : __("pos.invoice_word") }}</th><th class="py-2.5 px-3 hidden sm:table-cell">{{ $isRestaurant ? __("pos.type_word") : __("pos.customer_word") }}</th><th class="py-2.5 px-3 text-right">{{ __("pos.amount_word") }}</th><th class="py-2.5 px-3 text-center">{{ __("pos.status_word") }}</th></tr></thead><tbody>
            ...
            @empty<tr><td colspan="4" class="py-10 text-center text-[12px] text-gray-400">{{ $isRestaurant ? __("pos.no_orders_yet") : __("pos.no_transactions_yet") }}</td></tr>@endforelse
        </tbody></table>
    </div>
</div>
