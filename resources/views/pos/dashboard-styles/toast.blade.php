...
<div class="space-y-4 w-full">
    <div class="flex items-center justify-between t-anim t-d1">
        <div>
            <h1 class="text-lg font-extrabold text-gray-900 dark:text-white">{{ __("pos.dashboard") }}</h1>
            <p class="text-[11px] text-gray-400">{{ now()->format('D, d M Y') }} — {{ $company->name ?? __("pos.business_fallback") }}</p>
        </div>
    ...
    <div class="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 overflow-hidden t-anim t-d2">
        <div class="grid grid-cols-2 sm:grid-cols-4 divide-x divide-gray-100 dark:divide-gray-800">
            ...
            <div class="p-4 sm:p-5">
                <div class="flex items-center gap-1.5 mb-1"><span class="w-2 h-2 rounded-full bg-blue-500"></span><span class="text-[9px] font-bold text-gray-400 uppercase tracking-wider">{{ __("pos.orders_word") }}</span></div>
            ...
            <div class="p-4 sm:p-5">
                @if($isRestaurant)
                <div class="flex items-center gap-1.5 mb-1"><span class="w-2 h-2 rounded-full bg-emerald-500"></span><span class="text-[9px] font-bold text-gray-400 uppercase tracking-wider">{{ __("pos.tables_word") }}</span></div>
    ...
    <div class="bg-white dark:bg-gray-900 rounded-xl border border-gray-100 dark:border-gray-800 p-4 t-anim t-d3">
        <div class="flex items-center gap-6 flex-wrap">
            <div class="flex items-center gap-3 border-l-3 border-l-emerald-500 pl-3">
                <div><p class="text-[8px] text-gray-400 font-bold uppercase">{{ __("pos.gross_profit") }}</p><p class="text-base font-black {{ ($todayProfit ?? 0) >= 0 ? 'text-emerald-600' : 'text-red-600' }}">Rs.{{ number_format($todayProfit ?? 0) }}</p></div>
            </div>
            <div class="w-px h-8 bg-gray-100 dark:bg-gray-800"></div>
            <div class="flex items-center gap-3 border-l-3 border-l-gray-300 pl-3">
                <div><p class="text-[8px] text-gray-400 font-bold uppercase">{{ __("pos.total_cost") }}</p><p class="text-base font-black text-gray-900 dark:text-white">Rs.{{ number_format($todayCost ?? 0) }}</p></div>
            </div>
            <div class="w-px h-8 bg-gray-100 dark:bg-gray-800"></div>
            <div class="flex items-center gap-3 border-l-3 border-l-amber-500 pl-3">
                <div><p class="text-[8px] text-gray-400 font-bold uppercase">{{ __("pos.margin_word") }}</p><p class="text-base font-black text-amber-600">{{ ($todaySales ?? 0) > 0 ? round(($todayProfit ?? 0) / ($todaySales ?? 1) * 100) : 0 }}%</p></div>
            </div>
            <div class="w-px h-8 bg-gray-100 dark:bg-gray-800"></div>
            <div class="flex items-center gap-3 border-l-3 border-l-cyan-500 pl-3">
                <div><p class="text-[8px] text-gray-400 font-bold uppercase">{{ __("pos.tax_word") }}</p><p class="text-base font-black text-cyan-600">Rs.{{ number_format($todayTax ?? $todayStats->tax ?? 0) }}</p></div>
            </div>
            <div class="w-px h-8 bg-gray-100 dark:bg-gray-800"></div>
            <div class="flex items-center gap-3 border-l-3 border-l-orange-500 pl-3">
                <div><p class="text-[8px] text-gray-400 font-bold uppercase">{{ __("pos.discounts_word") }}</p><p class="text-base font-black text-orange-600">Rs.{{ number_format($todayDiscount ?? 0) }}</p></div>
            </div>
        </div>
    </div>
    @endif
    ...
    <div class="grid grid-cols-1 lg:grid-cols-5 gap-4 t-anim t-d3">
        <div class="lg:col-span-3 bg-white dark:bg-gray-900 rounded-xl border border-gray-100 dark:border-gray-800 p-4">
            <div class="flex items-center justify-between mb-3"><h2 class="text-[11px] font-bold text-gray-900 dark:text-white uppercase tracking-wide">{{ __("pos.revenue_trend") }}</h2><span class="text-[9px] text-gray-400">{{ __("pos.last_7_days") }}</span></div>
        ...
        <div class="lg:col-span-2 space-y-4">
            ...
            <div class="bg-white dark:bg-gray-900 rounded-xl border border-gray-100 dark:border-gray-800 p-4">
                <h2 class="text-[11px] font-bold text-gray-900 dark:text-white uppercase tracking-wide mb-3">{{ __("pos.order_mix") }}</h2>
                <div style="height: 140px;"><canvas id="orderTypeChart"></canvas></div>
            </div>
            @endif
            <div class="bg-white dark:bg-gray-900 rounded-xl border border-gray-100 dark:border-gray-800 p-4">
                <h2 class="text-[11px] font-bold text-gray-900 dark:text-white uppercase tracking-wide mb-3">{{ __("pos.stock_status") }}</h2>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-5 gap-4 t-anim t-d3">
        <div class="lg:col-span-2 bg-white dark:bg-gray-900 rounded-xl border border-gray-100 dark:border-gray-800 p-4">
            <h3 class="text-[11px] font-bold text-gray-900 dark:text-white uppercase tracking-wide mb-3">{{ __("pos.payment_breakdown") }}</h3>
            ...
            @empty
            <p class="text-[11px] text-gray-400 py-4 text-center">{{ __("pos.no_sales_today") }}</p>
            @endforelse
        </div>
        <div class="lg:col-span-3 bg-white dark:bg-gray-900 rounded-xl border border-gray-100 dark:border-gray-800 overflow-hidden">
            <div class="px-4 py-3 border-b border-gray-100 dark:border-gray-800 flex items-center justify-between">
                ...
                <a href="{{ route('pos.transactions') }}" class="text-[10px] font-bold text-amber-600 hover:text-amber-700">{{ __("pos.view_all_arrow_title") }}</a>
            </div>
            <table class="w-full"><thead><tr class="text-left text-[9px] font-bold text-gray-400 uppercase bg-gray-50/50 dark:bg-gray-800/30"><th class="py-2 px-4">{{ __("pos.invoice_word") }}</th><th class="py-2 px-3">{{ __("pos.customer_word") }}</th><th class="py-2 px-3 text-right">{{ __("pos.amount_word") }}</th><th class="py-2 px-3">{{ __("pos.time_word") }}</th></tr></thead><tbody>
                ...
                <tr class="border-b border-gray-50 dark:border-gray-800/50 last:border-0 hover:bg-amber-50/30 dark:hover:bg-amber-900/5 transition">
                    ...
                    <td class="py-2.5 px-3 text-[11px] text-gray-600 dark:text-gray-400">{{ $txn->customer_name ?? __("pos.walk_in") }}</td>
                ...
                @empty
                <tr><td colspan="4" class="py-6 text-center text-[11px] text-gray-400">{{ __("pos.no_transactions_yet") }}</td></tr>
                @endforelse
            </tbody></table>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-gray-900 rounded-xl border border-gray-100 dark:border-gray-800 t-anim t-d4">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 divide-y sm:divide-y-0 sm:divide-x divide-gray-50 dark:divide-gray-800">
            ...
            <div class="flex items-center gap-3 py-3 px-4 hover:bg-amber-50/30 dark:hover:bg-amber-900/5 transition">
                ...
                <div class="flex-1 min-w-0"><p class="text-[11px] font-bold text-gray-900 dark:text-white truncate">{{ $p->item_name }}</p><p class="text-[9px] text-gray-400">{{ $p->total_qty }} {{ __("pos.sold_suffix") }}</p></div>
                <p class="text-[11px] font-black text-gray-900 dark:text-white flex-shrink-0">Rs.{{ number_format($p->total_revenue) }}</p>
            </div>
            @empty
            <div class="col-span-2 py-8 text-center text-[11px] text-gray-400">{{ __("pos.no_sales_data") }}</div>
            @endforelse
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-gray-900 rounded-xl border border-gray-100 dark:border-gray-800 overflow-hidden t-anim t-d5">
        <div class="px-4 py-3 border-b border-gray-100 dark:border-gray-800 flex items-center justify-between">
            <h2 class="text-[11px] font-bold text-gray-900 dark:text-white uppercase">{{ __("pos.recent_orders") }}</h2>
            <a href="{{ route('pos.transactions') }}" class="text-[10px] font-bold text-amber-600">{{ __("pos.view_all_arrow_title") }}</a>
        </div>
        <table class="w-full"><thead><tr class="bg-gray-50/60 dark:bg-gray-800/30"><th class="text-left text-[9px] text-gray-400 font-bold uppercase py-2 px-4">{{ __("pos.order_word") }}</th><th class="text-left text-[9px] text-gray-400 font-bold uppercase py-2 px-3 hidden sm:table-cell">{{ __("pos.type_word") }}</th><th class="text-right text-[9px] text-gray-400 font-bold uppercase py-2 px-3">{{ __("pos.amount_word") }}</th><th class="text-center text-[9px] text-gray-400 font-bold uppercase py-2 px-3">{{ __("pos.status_word") }}</th></tr></thead><tbody>
            ...
            <tr class="border-b border-gray-50 dark:border-gray-800/50 hover:bg-amber-50/20 dark:hover:bg-amber-900/5 transition">
                <td class="py-2.5 px-4 text-amber-600">@include('pos.dashboard-styles._restaurant-order-identity', ['order' => $ro])</td>
            ...
            @empty
            <tr><td colspan="4" class="py-8 text-center text-[11px] text-gray-400">{{ __("pos.no_orders_today") }}</td></tr>
            @endforelse
        </tbody></table>
    </div>
    @endif
