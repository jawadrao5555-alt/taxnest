...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
    ...
    @include('pos.inventory.partials.nav-tabs', ['active' => 'low-stock'])
    ...
    <div class="bg-gradient-to-r from-red-50 to-rose-50 dark:from-red-900/20 dark:to-rose-900/20 border border-red-200 dark:border-red-700 rounded-2xl p-5 mb-6">
        <div class="flex items-center gap-2 mb-4">
            ...
            <h3 class="text-sm font-bold text-red-800 dark:text-red-300">{{ __('pos.out_of_stock') }} ({{ __('pos.n_products', ['count' => $outOfStock->count()]) }})</h3>
        </div>
    ...
    <div class="grid grid-cols-3 gap-3 mb-6">
        <div class="group bg-white dark:bg-gray-900 rounded-2xl border-2 {{ $criticalItems->count() > 0 ? 'border-red-300 dark:border-red-700' : 'border-gray-100 dark:border-gray-700' }} shadow-lg p-4 text-center relative overflow-hidden hover:shadow-xl hover:-translate-y-0.5 transition-all duration-300">
            ...
            <p class="text-[10px] font-bold uppercase tracking-wider {{ $criticalItems->count() > 0 ? 'text-red-500' : 'text-gray-400' }}">{{ __('pos.critical_under_25') }}</p>
        </div>
        <div class="group bg-white dark:bg-gray-900 rounded-2xl border-2 {{ $warningItems->count() > 0 ? 'border-amber-300 dark:border-amber-700' : 'border-gray-100 dark:border-gray-700' }} shadow-lg p-4 text-center relative overflow-hidden hover:shadow-xl hover:-translate-y-0.5 transition-all duration-300">
            ...
            <p class="text-[10px] font-bold uppercase tracking-wider {{ $warningItems->count() > 0 ? 'text-amber-500' : 'text-gray-400' }}">{{ __('pos.warning_25_50') }}</p>
        </div>
        <div class="group bg-white dark:bg-gray-900 rounded-2xl border-2 {{ $lowItems->count() > 0 ? 'border-yellow-300 dark:border-yellow-700' : 'border-gray-100 dark:border-gray-700' }} shadow-lg p-4 text-center relative overflow-hidden hover:shadow-xl hover:-translate-y-0.5 transition-all duration-300">
            ...
            <p class="text-[10px] font-bold uppercase tracking-wider {{ $lowItems->count() > 0 ? 'text-yellow-500' : 'text-gray-400' }}">{{ __('pos.low_50_100') }}</p>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-lg overflow-hidden">
        <div class="px-5 py-4 border-b border-gray-100 dark:border-gray-700 bg-gradient-to-r from-amber-50 to-orange-50 dark:from-amber-900/10 dark:to-orange-900/10">
            <div class="flex items-center gap-2">
                ...
                <h3 class="text-sm font-bold text-amber-800 dark:text-amber-300">{{ __('pos.below_min_stock_level') }} ({{ __('pos.n_products', ['count' => $alerts->count()]) }})</h3>
            </div>
        </div>
            ...
            <table class="w-full text-sm table-cards">
                <thead>
                    <tr class="text-left text-xs text-gray-500 dark:text-gray-400 uppercase border-b border-gray-100 dark:border-gray-700 bg-gray-50/80 dark:bg-gray-800/50">
                        ...
                        <th class="px-5 py-3.5 text-right font-semibold">{{ __('pos.shortage_col') }}</th>
                        <th class="px-5 py-3.5 text-center font-semibold">{{ __('pos.action_col') }}</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-50 dark:divide-gray-800">
                    ...
                    @php
                        ...
                        $urgencyLabel = $pct < 25 ? __('pos.urgency_critical') : ($pct < 50 ? __('pos.urgency_warning') : __('pos.urgency_low'));
    ...
    <div class="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-lg p-16 text-center">
        ...
        <p class="text-xl font-bold text-gray-900 dark:text-white">{{ __('pos.all_stock_healthy') }}</p>
        <p class="text-sm text-gray-500 mt-2">{{ __('pos.all_stock_healthy_sub') }}</p>
    </div>
    @endif
    @endif
</div>
