...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
    ...
    <div class="flex flex-wrap gap-2 mb-6">
        ...
        <a href="{{ route('pos.inventory.stock') }}" class="px-4 py-2 text-xs font-semibold rounded-xl bg-purple-600 text-white shadow-sm">{{ __('pos.stock_levels') }}</a>
    ...
    <div class="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-lg p-5 mb-6">
        <form method="GET" action="{{ route('pos.inventory.stock') }}" class="flex flex-col sm:flex-row gap-3">
            <div class="flex-1 relative">
                ...
                <input type="text" name="search" value="{{ request('search') }}" placeholder="{{ __('pos.ph_search_products_simple') }}" class="w-full pl-10 rounded-xl border border-gray-200 dark:border-gray-600 bg-gray-50 dark:bg-gray-800 text-gray-900 dark:text-gray-100 text-sm px-3 py-2.5 focus:ring-2 focus:ring-purple-500 focus:border-transparent transition">
            </div>
            <select name="filter" class="rounded-xl border border-gray-200 dark:border-gray-600 bg-gray-50 dark:bg-gray-800 text-gray-900 dark:text-gray-100 text-sm px-3 py-2.5 focus:ring-2 focus:ring-purple-500 transition">
                <option value="">{{ __('pos.all_stock') }}</option>
                <option value="low" {{ request('filter') === 'low' ? 'selected' : '' }}>{{ __('pos.low_stock') }}</option>
                <option value="out" {{ request('filter') === 'out' ? 'selected' : '' }}>{{ __('pos.out_of_stock') }}</option>
            </select>
            <button type="submit" class="px-5 py-2.5 bg-purple-600 hover:bg-purple-700 text-white text-sm font-semibold rounded-xl transition shadow-sm">{{ __('pos.filter_btn') }}</button>
        </form>
    </div>
            ...
            <table class="w-full text-sm table-cards">
                <thead>
                    <tr class="text-left text-xs text-gray-500 dark:text-gray-400 uppercase border-b border-gray-100 dark:border-gray-700 bg-gray-50/80 dark:bg-gray-800/50">
                        ...
                        <th class="px-5 py-3.5 text-right font-semibold hidden md:table-cell">{{ __('pos.stock_value') }}</th>
                        <th class="px-5 py-3.5 text-center font-semibold">{{ __('pos.status_col') }}</th>
                        <th class="px-5 py-3.5 text-right font-semibold hidden sm:table-cell">{{ __('pos.min_level_setting') }}</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-50 dark:divide-gray-800">
                    ...
                    <tr class="hover:bg-gray-50/80 dark:hover:bg-gray-800/30 transition group">
                        ...
                        <td class="px-5 py-4 text-right hidden sm:table-cell" x-data="{ editing: false, minLevel: {{ $stock->min_stock_level }}, saving: false }">
                            <template x-if="!editing">
                                <button @click="editing = true" class="text-xs text-purple-600 hover:text-purple-800 font-semibold transition">{{ __('pos.edit') }}</button>
                            </template>
                            <template x-if="editing">
                    ...
                    <tr>
                        <td colspan="{{ ($allBranches ?? false) ? 8 : 7 }}" class="px-5 py-16 text-center">
                            ...
                            <p class="text-sm font-medium text-gray-500">{{ __('pos.no_stock_records') }}</p>
                            <p class="text-xs text-gray-400 mt-1">{{ __('pos.add_stock_hint') }}</p>
                        </td>
                    </tr>
                    @endforelse
                </tbody>
