<div id="pinModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/50 backdrop-blur-sm" x-data="pinVerification()" x-cloak>
    <div class="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-700 p-8 w-full max-w-sm mx-4">
        ...
        <div class="flex gap-3">
            <button onclick="closePinModal()" class="flex-1 px-4 py-2.5 border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 rounded-xl text-sm font-semibold hover:bg-gray-50 dark:hover:bg-gray-800 transition">{{ __('pos.cancel') }}</button>
            <button onclick="submitPin()" id="pinSubmitBtn" class="flex-1 px-4 py-2.5 bg-purple-600 text-white rounded-xl text-sm font-semibold hover:bg-purple-700 transition disabled:opacity-50" disabled>{{ __('pos.verify') }}</button>
        </div>
    </div>
</div>
...
function submitPin() {
    ...
    document.getElementById('pinSubmitBtn').textContent = @js(__('pos.verifying_ellipsis'));
    ...
    .then(({ status, data }) => {
        ...
        document.getElementById('pinSubmitBtn').textContent = @js(__('pos.verify'));
    })
    .catch(() => {
        document.getElementById('pinError').textContent = @js(__('pos.network_error_try_again'));
        document.getElementById('pinError').classList.remove('hidden');
        document.getElementById('pinSubmitBtn').disabled = false;
        document.getElementById('pinSubmitBtn').textContent = @js(__('pos.verify'));
    });
