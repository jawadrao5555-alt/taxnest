...
<div class="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm p-5" data-rider-preview-settings>
    ...
    @foreach(['enabled' => 'Enable bill preview', 'quantity' => 'Quantity', 'prices' => 'Unit rate and line total', 'tax' => 'Tax breakup', 'ntn' => 'NTN', 'qr' => 'QR (currently unavailable)', 'customer' => 'Customer details', 'business' => 'Business details'] as $key => $label)
        <label class="flex items-center gap-2 text-sm text-gray-700 dark:text-gray-200"><input type="checkbox" name="{{ $key }}" value="1" {{ !empty($rbp[$key]) ? 'checked' : '' }} class="rounded text-purple-600"> {{ $label }}</label>
    @endforeach
    </div>
