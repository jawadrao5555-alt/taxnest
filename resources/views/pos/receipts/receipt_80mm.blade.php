    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; font-style: normal !important; }
        ...
        /* Urdu script mode (Task 240; JNN-first since Task 1287 — owner chose
           ...
           decision). Browser + Desktop Agent renders are Chromium — Arabic
           shaping & bidi work natively. Layout stays LTR (columns/widths
           untouched, thermal-print-width rules intact); Unicode bidi flips
           each Urdu text run RTL on its own, so mixed label/number lines still
           line up. Fallbacks keep Arabic coverage when the web font can't load
           (offline agent). Line-height 1.9: Nastaleeq stacks ligatures taller
           than Naskh — 1.6 clips its ascenders/descenders.
...
<body>
    ...
    @php
        ...
        // Aik hi faisla-gah (PosBillNumberStyle) — 'token' = 7, 'daily' = L007.
    ...
    {{-- Task 816: split-payment breakdown — printed receipt mirrors the public
         ...
         strict lazy-loading never throws. Mirrors receipt_58mm; keep in sync. --}}
    @php
        $rcptPayBreakdown = [];
        try {
