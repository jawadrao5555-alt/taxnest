...
<div x-data="restaurantPos()" class="flex flex-col h-[calc(100vh-48px)] overflow-hidden bg-gray-50 dark:bg-gray-950">
        ...
        <div class="relative flex-shrink-0" style="min-width:180px;max-width:220px;">
            ...
            <div x-show="customerPhoneDropdown && customerPhoneResults.length > 0" x-transition class="absolute top-full left-0 right-0 mt-1 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl shadow-2xl z-50 max-h-52 overflow-y-auto" style="min-width:280px;">
                <template x-for="(cr, ci) in customerPhoneResults" :key="cr.id">
                    <div class="w-full flex items-stretch border-b border-gray-50 dark:border-gray-800 hover:bg-blue-50 dark:hover:bg-blue-900/20 transition" :class="ci === 0 ? 'bg-blue-50/50 dark:bg-blue-900/10' : ''">
    ...
    <div x-show="guidedFlow" x-cloak class="flex items-center justify-center flex-wrap gap-1.5 px-3 py-1.5 bg-gradient-to-r from-emerald-50 to-blue-50 dark:from-emerald-900/20 dark:to-blue-900/20 border-b border-emerald-200 dark:border-emerald-800 flex-shrink-0 text-[11px] font-bold select-none pointer-events-none">
        <template x-for="(s, i) in [{k:'customer',l:'1 · Customer'},{k:'items',l:'2 · Items'},{k:'type',l:'3 · Type'},{k:'cart',l:'4 · Cart'},{k:'finish',l:'5 · Bill'}]" :key="s.k">
            <div class="flex items-center gap-1.5">
                ...
                <svg x-show="i < 4" class="w-3 h-3 text-gray-300 dark:text-gray-600" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
            </div>
        </template>
    </div>
    ...
    {{-- Owner-specified keyboard step BETWEEN Items and Cart. Reached by pressing Enter on an
         empty search box (cart already has items). Arrow keys move the highlight (handled in
         handleKey), Enter confirms the type + drops into the cart, Esc returns to product search. --}}
    <div x-cloak x-show="guidedFlow && flowStep === 'type'" x-transition.opacity class="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
        <div class="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl w-full max-w-md p-6 border border-gray-100 dark:border-gray-800">
            ...
            <div class="space-y-2">
                <template x-for="(t, i) in [{k:'dine_in',l:'Dine In'},{k:'takeaway',l:'Takeaway'},{k:'delivery',l:'Delivery'}]" :key="t.k">
    ...
    <div x-cloak x-show="showPayModal" x-transition.opacity x-effect="if (showPayModal) payMethodIndex = 0;" class="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4" @click.self="showPayModal = false">
        <div class="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden" x-transition.scale.90>
            ...
            <div class="p-4 grid grid-cols-2 gap-3">
                <button @click="payMethodIndex = 0; processPayment('cash')" :disabled="submitting" :class="payMethodIndex === 0 ? 'ring-2 ring-green-500 ring-offset-2 dark:ring-offset-gray-900 scale-105 shadow-lg shadow-green-600/20 border-green-400' : ''" class="py-4 rounded-xl text-center bg-green-50 dark:bg-green-900/20 border-2 border-green-200 dark:border-green-800 hover:bg-green-100 hover:border-green-400 transition disabled:opacity-50 group">
                ...
                <button @click="payMethodIndex = 1; processPayment('card')" :disabled="submitting" :class="payMethodIndex === 1 ? 'ring-2 ring-blue-500 ring-offset-2 dark:ring-offset-gray-900 scale-105 shadow-lg shadow-blue-600/20 border-blue-400' : ''" class="py-4 rounded-xl text-center bg-blue-50 dark:bg-blue-900/20 border-2 border-blue-200 dark:border-blue-800 hover:bg-blue-100 hover:border-blue-400 transition disabled:opacity-50 group">
    ...
    <div x-cloak x-show="showTablePicker" x-transition.opacity class="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4" @click.self="showTablePicker = false">
    ...
    <div x-cloak x-show="showHeldOrders" x-transition.opacity class="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4" @click.self="showHeldOrders = false">
    ...
    <div x-cloak x-show="showNewCustomerModal" x-transition.opacity class="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4" @click.self="showNewCustomerModal = false" @keydown.escape.window="if(showNewCustomerModal) showNewCustomerModal = false">
        <div class="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden" x-transition.scale.90>
            <div class="p-4 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between">
    ...
    <div x-cloak x-show="showCustomerPicker" x-transition.opacity class="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4" @click.self="showCustomerPicker = false">
    ...
    <div x-cloak x-show="showShortcuts" x-transition.opacity @click.self="showShortcuts = false" class="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4" style="display:none;">
                ...
                <div style="display:grid; grid-template-columns:1fr 1fr; gap:16px;">
                    <div>
                        ...
                        <div style="display:flex; flex-direction:column; gap:6px;">
                            ...
                            <div style="display:flex; align-items:center; justify-content:space-between; padding:6px 10px; background:#f9fafb; border-radius:8px;" class="dark:bg-gray-800">
                                <span style="font-size:12px; font-weight:600; color:#374151;" class="dark:text-gray-300">Edit Latest Item Quantity</span>
    ...
    {{-- ============================================================ --}}
    <div x-cloak x-show="showLastSaleWidget && !showReceipt && lastTransactionId" x-transition.opacity
    ...
    <div x-cloak x-show="showReceipt" x-transition.opacity x-effect="if (!showReceipt) cancelPendingPrints()" class="fixed inset-0 bg-gradient-to-br from-green-900/80 via-black/70 to-emerald-900/80 backdrop-blur-md z-50 flex items-center justify-center p-4">
        <div class="receipt-modal-enter relative bg-white dark:bg-gray-900 rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden" x-transition.scale.90>
            ...
            {{-- Header / success info --}}
            <div class="relative px-6 pt-7 pb-6 text-center overflow-hidden bg-gradient-to-b from-emerald-50 via-green-50 to-white dark:from-emerald-900/30 dark:via-green-900/10 dark:to-gray-900" id="confettiContainer">
    ...
    {{-- Manager PIN Modal --}}
    <div x-cloak x-show="showManagerPinModal" x-transition.opacity class="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
    ...
    <div x-cloak x-show="showCustomerHistory" x-transition.opacity class="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
...
function restaurantPos() {
    return {
        ...
        // move it, Enter confirms, number keys 1/2 jump + fire. Reset to 0 on open.
        payMethodIndex: 0,
        ...
        addHighlightedItem() {
            ...
            // already has items advances to the ORDER-TYPE step (dine in / takeaway / delivery)
            // — the owner-specified step between Items and Cart. The cashier picks the type with
            // arrows + Enter, which then drops focus into the cart ("double Enter" pattern: this
            // empty-search Enter, then Enter on the chosen type). Only when guided; otherwise the
            // manual-line fallback (incl. the blocking price prompt) runs exactly as before.
        ...
        removeFromCart(index) {
            ...
            // Capture stable cart_uid BEFORE setTimeout — index becomes stale
            // if any other cart mutation happens during the 250ms animation.
            const uid = this.cart[index].cart_uid;
            const el = this.$refs.cartList?.querySelector(`[data-cart-index="${index}"]`);
            if (el) {
                el.classList.add('cart-item-exit');
                setTimeout(() => {
                    // Re-find by uid (not stale index) at splice time
                    const realIdx = this.cart.findIndex(c => c.cart_uid === uid);
                    if (realIdx !== -1) {
                        this.cart.splice(realIdx, 1);
                        this.fixCartIndex();
                    }
                }, 250);
            } else {
        ...
        // between Items and Cart. enterTypeStep() seeds the highlight from the current orderType
        // and blurs the search box so handleKey owns arrows/Enter; the overlay (x-show
        // guidedFlow && flowStep==='type') renders the choices. confirmGuidedType() commits the
        // picked type and drops straight into cart mode. Gated on guidedFlow only.
        guidedOrderTypes() { return ['dine_in', 'takeaway', 'delivery']; },
        enterTypeStep() {
            this.flowStep = 'type';
        ...
        confirmGuidedType() {
            this.orderType = this.guidedOrderTypes()[this.flowTypeIndex] || 'takeaway';
            this.flowStep = 'cart';
            this.enterCartMode();
        },
        ...
        handleKey(e) {
            ...
            if (this.showPayModal) {
                ...
                if (e.key === 'ArrowLeft' || e.key === 'ArrowUp')   { e.preventDefault(); this.payMethodIndex = 0; }
                else if (e.key === 'ArrowRight' || e.key === 'ArrowDown') { e.preventDefault(); this.payMethodIndex = 1; }
                // Number keys jump straight to that method AND fire it (fast path).
                else if (e.key === '1') { e.preventDefault(); this.payMethodIndex = 0; this.processPayment('cash'); }
                else if (e.key === '2') { e.preventDefault(); this.payMethodIndex = 1; this.processPayment('card'); }
                // Enter confirms the CURRENTLY-HIGHLIGHTED method (Cash by default).
                else if (e.key === 'Enter' && !e.repeat) { e.preventDefault(); if (!this.submitting) this.processPayment(this.payMethodIndex === 1 ? 'card' : 'cash'); }
            ...
            // GUIDED FLOW: Order-Type step capture (dine in / takeaway / delivery). Arrow keys
            // move the highlight, 1/2/3 jump to a choice, Enter confirms + drops into the cart,
            // Esc returns to product search. Reserved browser hotkeys are swallowed so nothing
            // leaks behind the overlay. Gated on guidedFlow so plain mode is byte-identical.
            if (this.guidedFlow && this.flowStep === 'type') {
                ...
                if (e.key === '1') { e.preventDefault(); this.flowTypeIndex = 0; return; }
                if (e.key === '2') { e.preventDefault(); this.flowTypeIndex = 1; return; }
                if (e.key === '3') { e.preventDefault(); this.flowTypeIndex = 2; return; }
        ...
        async editCustomerName(cr) {
            ...
            const entered = window.prompt(@json(__('pos.edit_customer_name')), cr.name || '');
            if (entered === null) return;
            const name = entered.trim();
            if (!name) { this.showToast(@json(__('pos.customer_name_required')), 'error'); return; }
            ...
            try {
                ...
                if (!res.ok || !data.success) {
                    this.showToast(data.message || @json(__('pos.failed_update_customer_name')), 'error');
                    return;
                }
                ...
                this.showToast(@json(__('pos.customer_name_updated')), 'success');
            } catch (e) {
                this.showToast(@json(__('pos.network_error')), 'error');
            } finally {
