<x-pos-layout>
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-6" data-service-order="{{ $order->job_number }}">
    ...
    <div class="mt-4 bg-white dark:bg-gray-900 border dark:border-gray-800 rounded-xl shadow-sm p-5">
        ...
        @if($nextStatuses || \App\Services\PosServiceWorkflowProfiles::canTransition($profile, $order->status, 'cancelled'))
        <form method="POST" action="{{ route('pos.service-work-orders.transition', $order) }}" class="mt-5 flex flex-wrap items-end gap-3">@csrf
            <div class="flex-1 min-w-[180px]"><label class="block text-xs font-semibold mb-1 dark:text-gray-200">Update note</label><input name="note" class="w-full rounded-lg border-gray-300 dark:bg-gray-800 dark:text-white"></div>
