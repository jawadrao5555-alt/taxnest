<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div class="flex items-center gap-3">
                ...
                <h2 class="font-bold text-xl text-gray-800 dark:text-gray-100 leading-tight">Risk Explanation Report - {{ $report['period'] }}</h2>
            </div>
    ...
    <div class="py-8">
        <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="bg-white dark:bg-gray-900 rounded-xl shadow-sm border border-gray-100 p-8 mb-6">
                <div class="flex items-center justify-between mb-6">
                    <div>
                        <h3 class="text-2xl font-bold text-gray-900">{{ $report['company']->name }}</h3>
                        <p class="text-sm text-gray-500 dark:text-gray-400">NTN: {{ $report['company']->ntn }} | Period: {{ $report['period'] }}</p>
                    </div>
                ...
                <h4 class="text-lg font-semibold text-gray-800 dark:text-gray-100 mb-3">Compliance Flag Summary</h4>
                <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
                    <div class="p-4 rounded-lg {{ $report['flag_summary']['RATE_MISMATCH'] > 0 ? 'bg-red-50 border border-red-200' : 'bg-gray-50' }}">
                        ...
                        <p class="text-xs text-gray-600 dark:text-gray-400">Rate Mismatch</p>
                    </div>
                    <div class="p-4 rounded-lg {{ $report['flag_summary']['BUYER_RISK'] > 0 ? 'bg-red-50 border border-red-200' : 'bg-gray-50' }}">
                        ...
                        <p class="text-xs text-gray-600 dark:text-gray-400">Buyer Risk (S.23)</p>
                    </div>
                    <div class="p-4 rounded-lg {{ $report['flag_summary']['BANKING_RISK'] > 0 ? 'bg-red-50 border border-red-200' : 'bg-gray-50' }}">
                        ...
                        <p class="text-xs text-gray-600 dark:text-gray-400">Banking (S.73)</p>
                    </div>
                    <div class="p-4 rounded-lg {{ $report['flag_summary']['STRUCTURE_ERROR'] > 0 ? 'bg-red-50 border border-red-200' : 'bg-gray-50' }}">
                        ...
                        <p class="text-xs text-gray-600 dark:text-gray-400">Structure Error</p>
                    </div>
                </div>
                    ...
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        <thead class="bg-gray-50 dark:bg-gray-800">
                            <tr>
                                ...
                                <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">Score</th>
                                <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">Invoices</th>
                                <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">Rejected</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                            @foreach($report['risky_vendors'] as $vendor)
                            <tr>
                                <td class="px-4 py-3 text-sm font-mono text-gray-700 dark:text-gray-300">{{ $vendor->vendor_ntn }}</td>
                                <td class="px-4 py-3 text-sm text-gray-700 dark:text-gray-300">{{ $vendor->vendor_name ?? 'N/A' }}</td>
                                ...
                                <td class="px-4 py-3 text-sm text-gray-700 dark:text-gray-300">{{ $vendor->total_invoices }}</td>
                                <td class="px-4 py-3 text-sm text-gray-700 dark:text-gray-300">{{ $vendor->rejected_invoices }}</td>
                            </tr>
                            @endforeach
                        </tbody>
                    </table>
                ...
                <div class="border-t border-gray-200 dark:border-gray-700 pt-6">
                    <div class="flex items-center justify-between">
                        ...
                        <div class="text-right">
                            ...
                            <p class="text-xs font-mono text-gray-600 dark:text-gray-400 break-all max-w-md">{{ $report['report_hash'] }}</p>
                        </div>
                    </div>
                </div>
            </div>
