        ...
        <tr>
            ...
            <td style="width:20%; padding:4px;">
                <div class="summary-card">
                    <div class="value">PKR {{ number_format($totals['total_value'], 2) }}</div>
                    <div class="label">Total Value (Excl ST)</div>
                </div>
            </td>
            <td style="width:20%; padding:4px;">
                <div class="summary-card">
                    <div class="value">PKR {{ number_format($totals['total_sales_tax'], 2) }}</div>
                    <div class="label">Sales Tax</div>
                </div>
            </td>
            <td style="width:20%; padding:4px;">
                <div class="summary-card warning">
                    <div class="value">PKR {{ number_format($totals['total_wht'], 2) }}</div>
                    <div class="label">WHT Collected</div>
                </div>
            </td>
            <td style="width:20%; padding:4px;">
                <div class="summary-card highlight">
                    <div class="value">PKR {{ number_format($totals['total_net_receivable'], 2) }}</div>
        ...
        <table class="data-table">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="right">PKR {{ number_format($row->total_value, 2) }}</td>
                    <td class="right">PKR {{ number_format($row->total_sales_tax, 2) }}</td>
                    <td class="center"><span class="badge badge-amber">{{ number_format($row->avg_wht_rate, 1) }}%</span></td>
                    <td class="right bold amber">PKR {{ number_format($row->total_wht, 2) }}</td>
                    <td class="right bold">PKR {{ number_format($row->total_net_receivable, 2) }}</td>
                </tr>
                @endforeach
            </tbody>
            <tfoot>
                <tr>
                    ...
                    <td class="right">PKR {{ number_format($rows->sum('total_value'), 2) }}</td>
                    <td class="right">PKR {{ number_format($rows->sum('total_sales_tax'), 2) }}</td>
                    <td class="center">-</td>
                    <td class="right">PKR {{ number_format($rows->sum('total_wht'), 2) }}</td>
                    <td class="right">PKR {{ number_format($rows->sum('total_net_receivable'), 2) }}</td>
                </tr>
            </tfoot>
        </table>
        @endforeach
        ...
        <table class="data-table">
            ...
            <tfoot>
                <tr>
                    ...
                    <td class="right">PKR {{ number_format($totals['total_value'], 2) }}</td>
                    <td class="right">PKR {{ number_format($totals['total_sales_tax'], 2) }}</td>
                    <td class="center">-</td>
                    <td class="right">PKR {{ number_format($totals['total_wht'], 2) }}</td>
                    <td class="right">PKR {{ number_format($totals['total_net_receivable'], 2) }}</td>
                </tr>
            </tfoot>
            @endif
        </table>
