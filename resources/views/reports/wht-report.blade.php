    ...
    <div class="py-8">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    ...
                    <div class="flex items-center justify-between">
                        ...
                        <div class="flex items-center flex-wrap gap-2">
                            ...
                            <div x-data="{ open: false }" class="relative">
                                ...
                                <div x-show="open" @click.away="open = false" class="absolute right-0 mt-1 w-48 bg-white dark:bg-gray-800 rounded-lg shadow-lg border border-gray-200 dark:border-gray-700 z-50">
                                    <a href="{{ route('reports.wht.pdf', array_merge(request()->query(), ['view' => 'whole'])) }}" target="_blank" class="block px-4 py-2 text-xs text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-t-lg">Whole Report PDF</a>
                                    <a href="{{ route('reports.wht.pdf', array_merge(request()->query(), ['view' => 'partywise'])) }}" target="_blank" class="block px-4 py-2 text-xs text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-b-lg">Party-wise PDF</a>
                                </div>
                            </div>
                            @php
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                ...
                <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 p-5 text-center">
                    <p class="text-2xl font-bold text-gray-900 dark:text-white">{{ number_format($totals['total_value'], 2) }}</p>
                    <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">Total Value (Excl ST)</p>
                </div>
                <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 p-5 text-center">
                    <p class="text-2xl font-bold text-amber-600 dark:text-amber-400">{{ number_format($totals['total_wht'], 2) }}</p>
                    <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">Total WHT Collected</p>
                </div>
                <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 p-5 text-center">
                    <p class="text-2xl font-bold text-blue-700 dark:text-blue-400">{{ number_format($totals['total_net_receivable'], 2) }}</p>
                    ...
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        ...
                        <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                            ...
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition">
                                ...
                                <td class="px-4 py-3 text-sm text-right text-gray-700 dark:text-gray-300">{{ number_format($row->total_value, 2) }}</td>
                                <td class="px-4 py-3 text-sm text-right text-gray-700 dark:text-gray-300">{{ number_format($row->total_sales_tax, 2) }}</td>
                                ...
                                <td class="px-4 py-3 text-sm text-right font-semibold text-gray-900 dark:text-white">{{ number_format($row->total_net_receivable, 2) }}</td>
                            </tr>
                            @empty
                            <tr>
                        ...
                        <tfoot class="bg-emerald-50 dark:bg-emerald-900/20">
                            <tr class="font-bold">
                                ...
                                <td class="px-4 py-3 text-sm text-right text-emerald-800 dark:text-emerald-300">{{ number_format($totals['total_value'], 2) }}</td>
                                <td class="px-4 py-3 text-sm text-right text-emerald-800 dark:text-emerald-300">{{ number_format($totals['total_sales_tax'], 2) }}</td>
                                <td class="px-4 py-3 text-sm text-center text-emerald-800 dark:text-emerald-300">-</td>
                                <td class="px-4 py-3 text-sm text-right text-emerald-800 dark:text-emerald-300">{{ number_format($totals['total_wht'], 2) }}</td>
                                <td class="px-4 py-3 text-sm text-right text-emerald-800 dark:text-emerald-300">{{ number_format($totals['total_net_receivable'], 2) }}</td>
                            </tr>
                        </tfoot>
                        @endif
                    </table>
