...
#   Every NEW-SHA deploy MUST have a What's New announcement (AppUpdate row,
#   audience pos/all, is_published=1) created since the last deploy marker.
#   SAME-SHA refresh/rerun may pass when the marker already records this SHA
#   and deploy/elaan.yml's exact title still exists as published pos/all
#   (see scripts/lib/elaan-freshness-check.sh). Unrelated old rows do not count.
#
...
# Task 999: Elaan freshness gate (shared with CI).
# NEW SHA: published pos/all AppUpdate created after the last deploy marker.
# SAME SHA: marker commit must match this SHA and deploy/elaan.yml title must
# still exist as published pos/all. See scripts/lib/elaan-freshness-check.sh.
# First-use bootstrap: no marker on live → FAIL; use --no-elaan once to seed.
check_elaan_freshness() {
  # LOCAL_HEAD is the manual deploy target (workspace HEAD after any reconcile).
  elaan_freshness_check "$LIVE_HEAD_BEFORE" "$LOCAL_HEAD"
}
