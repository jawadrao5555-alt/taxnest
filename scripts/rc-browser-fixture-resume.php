...
if((string)DB::connection()->getDriverName()!=='mysql'||(string)DB::connection()->getDatabaseName()!=='taxnest_rc_browser'||(string)config('database.connections.mysql.host')!=='127.0.0.1'||(string)config('database.connections.mysql.unix_socket')!==$socket||!is_file($socket)||is_link($socket)||!preg_match('#^/tmp/taxnest-rc-browser-[0-9]+/safe-runtime/mariadb/run/mariadb\.sock$#',$socket)||!preg_match('#^/tmp/taxnest-rc-browser-[0-9]+/safe-runtime/browser-state/fixture\.json$#',$out))$fail('connection or fixture target outside exact isolated allowlist.');
...
$users=User::whereIn('email',$emails)->get()->keyBy('email'); $categoryUsers=User::where('email','like','category-user-%@rc-browser.invalid')->get()->keyBy('email'); $expectedCategoryUsers=count(PosFeatureService::categories('pra'))+count(PosFeatureService::categories('fbr'))+2;$admin=AdminUser::where('email','hotel-admin-manage-as@rc-browser.invalid')->first();
if($users->count()!==count($emails)||$categoryUsers->count()!==$expectedCategoryUsers||!$admin)$fail('synthetic browser identities absent; setup required.');$users=$users->merge($categoryUsers);
...
foreach(['pra'=>'pos','fbr'=>'fbrpos'] as $panel=>$product) {
    foreach(array_merge(PosFeatureService::categories($panel),['general']) as $category) {
        ...
        $path=$landing==='hotel_front_desk'?'/pos/hotel':($landing==='service_work_orders'?'/pos/work-orders':($panel==='fbr'?'/fbr-pos/create':'/pos/invoice/create'));
        $marker=$panel==='fbr'?'New FBR POS Sale':($landing==='hotel_front_desk'?'Front Desk':($landing==='service_work_orders'?PosServiceWorkflowProfiles::forCompany($profileCompany)['noun'].' Board':'Current Order'));
        $categoryJourneys[]=['name'=>"category-$panel-$category",'login'=>$u("category-user-$panel-$category@rc-browser.invalid"),'password'=>$password,'loginPath'=>$panel==='fbr'?'/fbr-pos/login':'/pos/login','paths'=>[$path],'expectedPaths'=>[$path],'markers'=>[$marker],'mainMarkers'=>[$marker],'categoryCoverage'=>['panel'=>$panel,'category'=>$category,'landing'=>$landing,'mismatchPath'=>$landing==='service_work_orders'?'/pos/hotel':'/pos/work-orders']];
    }
}
$baseJourneys = [
