...
if((string)DB::connection()->getDriverName()!=='mysql'||(string)DB::connection()->getDatabaseName()!=='taxnest_rc_browser'||(string)config('database.connections.mysql.host')!=='127.0.0.1'||(string)config('database.connections.mysql.unix_socket')!==$socket||!is_file($socket)||is_link($socket)||!preg_match('#^/tmp/taxnest-rc-mariadb-browser-[0-9]+/run/mariadb\.sock$#',$socket)||!preg_match('#^/tmp/taxnest-rc-browser-[0-9]+/fixture\.json$#',$out))$fail('connection or fixture target outside exact isolated allowlist.');
