...
if((string)DB::connection()->getDriverName()!=='mysql'||(string)DB::connection()->getDatabaseName()!=='taxnest_rc_browser'||(string)config('database.connections.mysql.host')!=='127.0.0.1'||(string)config('database.connections.mysql.unix_socket')!==$socket||@filetype($socket)!=='socket'||is_link($socket)||!preg_match('#^/tmp/taxnest-rc-mariadb-browser-[0-9]+/run/mariadb\.sock$#',$socket)||!preg_match('#^/tmp/taxnest-rc-browser-[0-9]+(?:-[A-Za-z0-9_.-]+)?/safe-runtime/browser-state/fixture\.json$#',$out)||is_link($out)||is_link($out.'.di.tmp'))$fail('connection or fixture target outside exact isolated allowlist.');
...
$u=static fn(string $email)=>$users[$email]->email; $hotel=(int)$companies['hotel-company@rc-browser.invalid'];$fiscal=(int)$companies['fiscal-company@rc-browser.invalid'];$patientIds=DB::table('health_patients')->whereIn('mrn',['RC-OWN-001','RC-OTHER-BRANCH-001','RC-ISOLATED-001'])->pluck('id','mrn');if($patientIds->count()!==3)$fail('synthetic patient isolation rows absent; setup required.');
$categoryJourneys=[];
...
$baseJourneys = [
['name'=>'hotel-owner','login'=>$u('hotel-owner@rc-browser.invalid'),'password'=>$password,'loginPath'=>'/pos/login','paths'=>['/pos/hotel'],'markers'=>['Front Desk'],'usableSelectors'=>['[data-hotel-desk-menu="1"] a[data-hotel-desk-link="checkin"]','[data-hotel-reception-summary="1"]','[data-hotel-room-check-in]']],
['name'=>'pra-topnav','login'=>$u('hotel-owner@rc-browser.invalid'),'password'=>$password,'loginPath'=>'/pos/login','paths'=>['/pos/invoice/create'],'markers'=>['Current Order'],'mainMarkers'=>['Current Order'],'topNavPanel'=>'pra','topNavFactory'=>'restaurantPos'],
['name'=>'pra-admin-view-topnav','login'=>$admin->email,'password'=>$password,'loginPath'=>'/admin/login','submitPath'=>"/admin/companies/$hotel",'submitSelector'=>'form[action$="/impersonate"]:has(input[name="mode"][value="view"])','paths'=>['/pos/invoice/create'],'markers'=>['Current Order'],'mainMarkers'=>['Current Order'],'topNavPanel'=>'pra','topNavFactory'=>'restaurantPos'],
['name'=>'pra-admin-manage-topnav','login'=>$admin->email,'password'=>$password,'loginPath'=>'/admin/login','submitPath'=>"/admin/companies/$hotel",'submitSelector'=>'form[action$="/impersonate"]:has(input[name="mode"][value="full"])','paths'=>['/pos/invoice/create'],'markers'=>['Current Order'],'mainMarkers'=>['Current Order'],'topNavPanel'=>'pra','topNavFactory'=>'restaurantPos'],
...
['name'=>'hotel-admin-manage-as','login'=>$admin->email,'password'=>$password,'loginPath'=>'/admin/login','submitPath'=>"/admin/companies/$hotel",'submitSelector'=>'form[action$="/impersonate"]:has(input[name="mode"][value="full"])','paths'=>['/pos/hotel'],'markers'=>['Front Desk'],'usableSelectors'=>['[data-hotel-reception-summary="1"]','[data-hotel-room-check-in]']],
...
['name'=>'fiscal','login'=>$u('fiscal@rc-browser.invalid'),'password'=>$password,'loginPath'=>'/fbr-pos/login','paths'=>['/fbr-pos/create'],'markers'=>['Current Order'],'mainMarkers'=>['Current Order'],'usableSelectors'=>['input[name="pos_product_search_nofill"]'],'topNavPanel'=>'fbr','topNavFactory'=>'restaurantPos'],
['name'=>'fiscal-admin-view-topnav','login'=>$admin->email,'password'=>$password,'loginPath'=>'/admin/login','submitPath'=>"/admin/companies/$fiscal",'submitSelector'=>'form[action$="/impersonate"]:has(input[name="mode"][value="view"])','paths'=>['/fbr-pos/create'],'markers'=>['Current Order'],'mainMarkers'=>['Current Order'],'topNavPanel'=>'fbr','topNavFactory'=>'restaurantPos'],
['name'=>'fiscal-admin-manage-topnav','login'=>$admin->email,'password'=>$password,'loginPath'=>'/admin/login','submitPath'=>"/admin/companies/$fiscal",'submitSelector'=>'form[action$="/impersonate"]:has(input[name="mode"][value="full"])','paths'=>['/fbr-pos/create'],'markers'=>['Current Order'],'mainMarkers'=>['Current Order'],'topNavPanel'=>'fbr','topNavFactory'=>'restaurantPos'],
