#!/usr/bin/env bash
# Hotel room-overlap concurrency check against disposable local MariaDB.
# Does NOT change phpunit.xml (which remains sqlite :memory:).
# Safe local-only: taxnest_dev via unix socket under .local/mariadb.
...
if [[ ! -S "$SOCK" ]]; then
  ...
  echo "Start workspace MariaDB first (see docs/ops/cloud-agent-development.md)." >&2
  exit 2
fi
...
php "$ROOT/scripts/tests/hotel-mariadb-concurrency-check.php"
status=$?
exit $status
