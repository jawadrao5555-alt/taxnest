...
/**
 * FBR DAY-CLOSE RIDER FIGURES (Task 541 — mirror of PosRiderDayCloseFiguresTest).
 *
 * buildFbrRiderDayFigures() ka contract:
 ...
 *   - cash_in HYBRID hai:
 *       (a) legacy bill-based: sirf NULL-allocation settlements (pre-Task-525)
 *           ke bills — poora total_amount jab bill aaj settle hua aur
 *           business_date < aaj.
 *       (b) allocation-based: panel='fbr' settlement rows ki entries jin ka
 *           business_date < aaj — exact received rupees.
 *     Kabhi double-count nahi (legacy query allocation-wale settlements ke
 *     bills EXCLUDE karta hai).
 *   - FBR-set-only: invoice_mode 'fbr'/NULL; local provisionals excluded.
 *   - panel='pra' settlements FBR recon mein nahi aate.
 *
 * Pattern: sqlite :memory:, minimal Schema::create, private method via
 * reflection — same approach as PosRiderDayCloseFiguresTest.
 */
class FbrRiderDayCloseFiguresTest extends TestCase
{
    ...
    private function makeBill(int $riderId, string $businessDate, array $attrs = []): int
    {
        return (int) DB::table('fbr_pos_transactions')->insertGetId(array_merge([
            'company_id' => self::COMPANY,
            'invoice_number' => 'FBR-2026-' . uniqid(),
            'status' => 'completed',
            'invoice_mode' => 'fbr',
    ...
    private function makeSettlement(int $riderId, float $received, ?array $allocation, ?string $panel = 'fbr'): int
    {
    ...
    /** Invoke the private buildFbrRiderDayFigures via reflection. */
    private function figures(string $date): array
    {
        $controller = app(\App\Http\Controllers\FbrPosController::class);
        $m = new \ReflectionMethod($controller, 'buildFbrRiderDayFigures');
    ...
    public function test_partial_receipt_against_earlier_day_bills_counts_only_received_cash(): void
    {
        ...
        DB::table('fbr_pos_transactions')->where('id', $b1)
            ->update(['rider_settlement_id' => $sid, 'rider_settled_at' => now()]);
        DB::table('fbr_pos_transactions')->where('id', $b2)
    ...
    public function test_same_day_partial_keeps_cash_in_zero_and_cash_out_remaining(): void
    {
        ...
        DB::table('fbr_pos_transactions')->where('id', $bill)->update(['rider_partial_paid' => 200]);
    ...
    public function test_legacy_null_allocation_settlement_counts_bill_totals(): void
    {
        ...
        DB::table('fbr_pos_transactions')->where('id', $bill)
            ->update(['rider_settlement_id' => $sid, 'rider_settled_at' => now()]);
    ...
    public function test_mixed_legacy_and_allocation_settlements_never_double_count(): void
    {
        ...
        DB::table('fbr_pos_transactions')->where('id', $legacyBill)
        ...
        DB::table('fbr_pos_transactions')->where('id', $newBill)->update(['rider_partial_paid' => 250]);
    ...
    // ── 5. PRA-panel settlements / local provisionals stay out of the recon ──

    public function test_pra_panel_settlements_and_local_bills_are_excluded(): void
    {
        $today = now()->toDateString();
        $yesterday = now()->subDay()->toDateString();
        $riderId = $this->makeRider();

        // PRA-panel allocation settlement must NOT leak into FBR cash_in.
        $praBill = $this->makeBill($riderId, $yesterday, ['total_amount' => 600]);
        $this->makeSettlement($riderId, 600, [
            ['bill_id' => $praBill, 'amount' => 600, 'business_date' => $yesterday, 'full' => true],
        ], 'pra');

        // Local provisional cash bill today must NOT join cash_out (FBR-set-only).
        $this->makeBill($riderId, $today, [
            'total_amount' => 999, 'invoice_mode' => 'local', 'fbr_status' => 'local',
        ]);

        $f = $this->figures($today);

        $this->assertSame(0.0, $f['cash_in'], "panel='pra' settlements excluded");
        $this->assertSame(0.0, $f['cash_out'], 'local provisionals excluded from FBR recon');
    }
}
