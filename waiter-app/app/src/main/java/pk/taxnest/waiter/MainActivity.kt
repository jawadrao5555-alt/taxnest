 ...
 * in with normal web credentials; PosAuth recognises pos_role='pos_waiter' and
 * routes them to /pos/waiter automatically.  Unauthenticated visits are
 * redirected to /pos/login by the middleware, which then redirects back here
 * after a successful login.
 *
 * Native handling (copied verbatim from pos-app per download-cookie rule):
 *  - Downloads via DownloadManager WITH session cookie for first-party URLs,
 *    NEVER for agent-installer paths (those 302 to GitHub and must stay cookie-free).
 *  - target=_blank popups routed back into the same WebView.
 *  - File uploads via the system picker.
 *  - External schemes (tel:, WhatsApp, maps) → system apps.
 *  - Back = web history (never exits by accident mid-order).
 *  - configChanges in manifest prevent reload on rotation (protects mid-order state).
 *  - Offline error page in Urdu with retry button.
 */
class MainActivity : Activity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        with(web.settings) {
            ...
            userAgentString = "$userAgentString TaxNestWaiterApp/${BuildConfig.VERSION_NAME}"
        }
        ...
        // ── Download cookie rule (verbatim from pos-app v1.0.1) ───────────────
